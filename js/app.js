...
function compactGeneratedLine(value,maxChars=118){
 ...
 if(!text)return "";
 text=text.replace(/\b(con un lavoro|con spiegazione pratica|che desiderano)\b.*$/i,"").trim();
 if(text.length>maxChars){
  const cut=text.slice(0,maxChars+1),last=Math.max(cut.lastIndexOf(";"),cut.lastIndexOf(","),cut.lastIndexOf(" "));
  text=(last>55?cut.slice(0,last):text.slice(0,maxChars)).trim()+"…";
 }
 return text.replace(/[.;]+$/,'');
}
...
async function renderDesign(canvas,d){
 ...
 function cardGrid(x,y,bw,bh,cols=2,limit=4,mode="cards"){
  ...
  const baseCap=ratio>2?2:ratio>1.2?3:ratio<.82?4:3;
  const formatCap=d.contentMode==="essential"?Math.min(2,baseCap):d.contentMode==="compact"?Math.min(3,baseCap):baseCap;
  const requested=Math.max(1,Math.min(Number(limit)||formatCap,formatCap));
  const items=rawSections.slice(0,Math.min(requested,rawSections.length));
  if(!items.length)return;
  cols=ratio<.82?1:Math.min(cols,items.length,2);
  box("details",x+o.x,y+o.y,bw,bh);
  const gap=Math.max(8,unit*.009),rows=Math.ceil(items.length/cols),cw=(bw-gap*(cols-1))/cols;
  const pad=Math.max(10,unit*.012),headSize=Math.max(13,unit*.017);
  const maxBody=Math.max(17,unit*(ratio>1.2?.024:.027)),absoluteMin=Math.max(12,unit*.014);
  let bodySize=maxBody;
  const compute=size=>{
   const lines=items.map(([,body])=>{ctx.font=`600 ${size}px Arial,sans-serif`;return wrapAllLines(ctx,String(body||""),cw-pad*2)});
   const natural=lines.map(v=>pad+headSize*1.35+Math.max(1,v.length)*size*1.18+pad),heights=[];
   ...
   return{lines,heights,total:heights.reduce((a,b)=>a+b,0)+gap*(rows-1)};
  };
  let layout=compute(bodySize);while(layout.total>bh&&bodySize>absoluteMin){bodySize--;layout=compute(bodySize)}
  let rowHeights=layout.heights;if(layout.total<bh){const extra=(bh-layout.total)/rows;rowHeights=rowHeights.map(v=>v+extra)}
  items.forEach(([head],i)=>{
   const row=Math.floor(i/cols),col=i%cols,cx=x+o.x+col*(cw+gap),cy=y+o.y+rowHeights.slice(0,row).reduce((a,b)=>a+b,0)+gap*row,ch=rowHeights[row];ctx.save();
   ...
   else{ctx.fillStyle=p.surface;ctx.globalAlpha=.97;rr(cx,cy,cw,ch,unit*.009);ctx.fill();ctx.globalAlpha=1;ctx.strokeStyle=p.accent;ctx.lineWidth=Math.max(3,unit*.004);ctx.beginPath();ctx.moveTo(cx,cy);ctx.lineTo(cx,cy+ch);ctx.stroke()}
   ctx.fillStyle=p.accent;ctx.font=`900 ${headSize}px Arial,sans-serif`;ctx.textAlign="left";ctx.textBaseline="top";ctx.fillText(head,cx+pad,cy+pad);
   const bodyTop=cy+pad+headSize*1.35;ctx.fillStyle=p.ink;ctx.font=`600 ${bodySize}px Arial,sans-serif`;
   layout.lines[i].forEach((line,j)=>ctx.fillText(line,cx+pad,bodyTop+j*bodySize*1.18));ctx.restore();
  });
 }
 function footer(){
 ...
 if(rawSections.length&&!canvas._elementBoxes.details){
  const fallbackCount=ratio>2?3:ratio>1.2?4:ratio<.82?5:4;
