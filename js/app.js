...
function compatibleThemes(type){
 const all=[...$("style").options].map(o=>o.value||o.text).filter(Boolean), preferred=FORMAT_THEME_GROUPS[formatClass(type)]||[];
 const filtered=preferred.filter(name=>all.includes(name));
...
async function renderDesign(canvas,d){
 const f=formats[d.type]||formats["Post Instagram quadrato"],w=f.w,h=f.h,ctx=canvas.getContext("2d"),unit=Math.min(w,h),m=unit*.055;
 canvas.width=w;canvas.height=h;canvas._elementBoxes={};canvas._ctaBox=null;
 const img=await loadImg(d.image),logos=[];
 if(d.logo)logos.push({key:"logo",src:d.logo,pos:"br"});if(d.logo2)logos.push({key:"logo2",src:d.logo2,pos:d.logo2Position||"tl"});
 (d.extraLogos||[]).forEach((l,i)=>l?.data&&logos.push({key:`logo${i+3}`,src:l.data,pos:l.pos||"tr"}));
 const loadedLogos=await Promise.all(logos.map(async l=>({...l,img:await loadImg(l.src)})));
 ...
 const esc=k=>Math.max(.55,Math.min(1.55,Number((d.elementScales||elementScales)[k]||100)/100));
 const box=(k,x,y,bw,bh)=>canvas._elementBoxes[k]={x,y,w:bw,h:bh};
 const rounded=(x,y,bw,bh,r)=>{ctx.beginPath();ctx.roundRect?ctx.roundRect(x,y,bw,bh,r):(ctx.rect(x,y,bw,bh));};
 const model=normalizeGraphicVariant(d.type,d.graphicVariant??graphicVariant)%10;
 const themeIndex=Math.max(0,compatibleThemes(d.type).indexOf(d.style));
 const palettes=[
  {bg:'#f7f3ea',ink:'#122033',muted:'#4c5a68',accent:d.accent||'#ff7a00',surface:'rgba(255,255,255,.94)',line:'#e2d4bd'},
  {bg:'#07131d',ink:'#f7fafc',muted:'#c4d0d8',accent:d.accent||'#ff7a00',surface:'rgba(8,18,27,.90)',line:'rgba(255,255,255,.16)'},
  {bg:'#eaf4ee',ink:'#17352b',muted:'#4d695f',accent:'#3d8b62',surface:'rgba(255,255,255,.92)',line:'#bed8c8'},
  {bg:'#fff5ef',ink:'#412018',muted:'#72564e',accent:'#d9532f',surface:'rgba(255,255,255,.94)',line:'#efc7b7'},
  {bg:'#f2eef8',ink:'#211a36',muted:'#625978',accent:'#7554c9',surface:'rgba(255,255,255,.92)',line:'#d8ccef'},
  {bg:'#111111',ink:'#fff7df',muted:'#dacda9',accent:'#d6a84a',surface:'rgba(20,20,20,.92)',line:'#6f592c'}
 ];
 const p=palettes[(themeIndex+model)%palettes.length];
 const title=d.title||'Titolo della locandina',subtitle=d.subtitle||d.slogan||d.details||'',badge=`${d.discipline||'K9'}${d.objective?' · '+d.objective:''}`.toUpperCase();
 const cta=(d.ctaText||d.objective||'SCOPRI DI PIÙ').toUpperCase();
 const sections=[
  ['COSA COMPRENDE',d.benefits||d.details],['COME FUNZIONA',d.program||d.methodText],['PER CHI È INDICATO',d.targetText||d.audience],['METODO DI LAVORO',d.methodText||d.requirementsText]
 ].filter(x=>x[1]);
 function fitText(text,maxWidth,maxHeight,maxSize,minSize=12,weight=700,family='Arial'){
  let size=maxSize,lines=[];for(;size>=minSize;size-=1){ctx.font=`${weight} ${size}px ${family}`;lines=wrapAllLines(ctx,text,maxWidth);if(lines.length*size*1.12<=maxHeight)break}return{size,lines};
 }
 function drawLines(lines,x,y,size,color,align='left',lh=1.12){ctx.fillStyle=color;ctx.textAlign=align;ctx.textBaseline='top';for(let i=0;i<lines.length;i++)ctx.fillText(lines[i],x,y+i*size*lh)}
 function drawImage(x,y,bw,bh,darken=0){
  const o=off('image'),scale=esc('image');box('image',x,y,bw,bh);ctx.save();rounded(x,y,bw,bh,unit*.012);ctx.clip();ctx.fillStyle=p.bg;ctx.fillRect(x,y,bw,bh);
  if(img){const base=Math.max(bw/img.width,bh/img.height),sc=base*scale,dw=img.width*sc,dh=img.height*sc;ctx.drawImage(img,x+(bw-dw)/2+o.x,y+(bh-dh)/2+o.y,dw,dh)}
  if(darken){ctx.fillStyle=`rgba(0,0,0,${darken})`;ctx.fillRect(x,y,bw,bh)}ctx.restore();
 }
 function textBlock(k,text,x,y,bw,bh,maxSize,minSize,weight,color,align='left'){
  const o=off(k),sc=esc(k),r=fitText(text,bw,bh,maxSize*sc,minSize,weight);box(k,x+o.x,y+o.y,bw,bh);drawLines(r.lines,x+o.x+(align==='center'?bw/2:0),y+o.y,r.size,color,align);return r;
 }
 function badgeDraw(x,y,bw){const o=off('badge'),hh=unit*.045*esc('badge');ctx.save();ctx.fillStyle=p.accent;rounded(x+o.x,y+o.y,bw,hh,hh/2);ctx.fill();ctx.fillStyle='#fff';ctx.font=`800 ${Math.max(12,unit*.017*esc('badge'))}px Arial`;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(badge,x+o.x+bw/2,y+o.y+hh/2);ctx.restore();box('badge',x+o.x,y+o.y,bw,hh)}
 function buttonDraw(x,y,bw){const o=off('cta'),hh=unit*.065*esc('cta');ctx.save();ctx.fillStyle=p.accent;rounded(x+o.x,y+o.y,bw,hh,unit*.014);ctx.fill();ctx.fillStyle='#fff';ctx.font=`900 ${Math.max(13,unit*.021*esc('cta'))}px Arial`;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(cta,x+o.x+bw/2,y+o.y+hh/2);ctx.restore();box('cta',x+o.x,y+o.y,bw,hh);const url=resolveCtaUrl(d);if(url)canvas._ctaBox={x:x+o.x,y:y+o.y,w:bw,h:hh,url}}
 function cards(x,y,bw,bh,cols=2,limit=4,light=true){
  const o=off('details'),items=sections.slice(0,limit),gap=unit*.014,rows=Math.ceil(items.length/cols),cw=(bw-gap*(cols-1))/cols,ch=(bh-gap*(rows-1))/rows;box('details',x+o.x,y+o.y,bw,bh);
  items.forEach((it,i)=>{const cx=x+o.x+(i%cols)*(cw+gap),cy=y+o.y+Math.floor(i/cols)*(ch+gap);ctx.fillStyle=light?p.surface:'rgba(0,0,0,.38)';ctx.strokeStyle=p.line;ctx.lineWidth=Math.max(1,unit*.0015);rounded(cx,cy,cw,ch,unit*.012);ctx.fill();ctx.stroke();const pad=unit*.018;const hs=Math.max(13,Math.min(unit*.023,cw*.052));ctx.font=`900 ${hs}px Arial`;ctx.fillStyle=p.accent;ctx.textAlign='left';ctx.textBaseline='top';const hl=wrapLines(ctx,it[0],cw-pad*2,2);drawLines(hl,cx+pad,cy+pad,hs,p.accent,'left',1.05);const bodyY=cy+pad+hl.length*hs*1.08+unit*.006;const body=fitText(it[1],cw-pad*2,ch-(bodyY-cy)-pad,Math.max(12,unit*.015),Math.max(9,unit*.010),500);drawLines(body.lines,cx+pad,bodyY,body.size,light?p.ink:'#fff','left',1.18)})
 }
 function footerDraw(){const o=off('footer'),txt=[d.date,d.location,d.contact].filter(Boolean).join(' · ');if(!txt)return;ctx.font=`700 ${Math.max(11,unit*.014*esc('footer'))}px Arial`;ctx.fillStyle=p.muted;ctx.textAlign='center';ctx.textBaseline='bottom';ctx.fillText(txt,w/2+o.x,h-m*.35+o.y);box('footer',m,h-m*.8,w-m*2,m*.6)}
 function drawLogos(){loadedLogos.forEach((l,i)=>{if(!l.img)return;const key=l.key,sc=esc(key),base=unit*.095*sc,ratio=l.img.width/l.img.height,bw=ratio>=1?base:base*ratio,bh=ratio>=1?base/ratio:base;const pad=m*.45;let x=w-pad-bw,y=pad;if(l.pos==='tl'){x=pad;y=pad}else if(l.pos==='tr'){x=w-pad-bw;y=pad}else if(l.pos==='bl'){x=pad;y=h-pad-bh}else if(l.pos==='bc'){x=(w-bw)/2;y=h-pad-bh}else if(l.pos==='tc'){x=(w-bw)/2;y=pad}else{x=w-pad-bw;y=h-pad-bh}const o=off(key);x+=o.x;y+=o.y;ctx.save();ctx.fillStyle='rgba(255,255,255,.92)';rounded(x-unit*.008,y-unit*.008,bw+unit*.016,bh+unit*.016,unit*.012);ctx.fill();ctx.drawImage(l.img,x,y,bw,bh);ctx.restore();box(key,x-unit*.008,y-unit*.008,bw+unit*.016,bh+unit*.016)})}
 ctx.fillStyle=p.bg;ctx.fillRect(0,0,w,h);
 // 10 composizioni realmente differenti.
 if(model===0){ // Editoriale diviso
  drawImage(w*.48,0,w*.52,h,0);ctx.fillStyle=p.bg;ctx.fillRect(0,0,w*.50,h);ctx.fillStyle=p.accent;ctx.fillRect(w*.49,0,unit*.006,h);badgeDraw(m,m,w*.32);textBlock('title',title,m,h*.20,w*.38,h*.28,unit*.060,unit*.030,900,p.ink);textBlock('subtitle',subtitle,m,h*.51,w*.36,h*.16,unit*.023,unit*.014,500,p.muted);buttonDraw(m,h*.76,w*.30);
 }else if(model===1){ // Hero fotografico
  drawImage(0,0,w,h,.38);const g=ctx.createLinearGradient(0,h*.35,0,h);g.addColorStop(0,'rgba(0,0,0,0)');g.addColorStop(1,'rgba(0,0,0,.86)');ctx.fillStyle=g;ctx.fillRect(0,0,w,h);badgeDraw(m,m,w*.30);textBlock('title',title,m,h*.48,w-m*2,h*.22,unit*.068,unit*.032,900,'#fff');textBlock('subtitle',subtitle,m,h*.70,w*.68,h*.10,unit*.022,unit*.014,500,'#fff');buttonDraw(m,h*.84,w*.34);
 }else if(model===2){ // Poster tipografico
  ctx.fillStyle=p.accent;ctx.fillRect(0,0,w*.10,h);drawImage(w*.58,m,w*.36,h-m*2,0);badgeDraw(m*.6,m,w*.34);textBlock('title',title,w*.14,h*.18,w*.39,h*.42,unit*.080,unit*.034,900,p.ink);textBlock('subtitle',subtitle,w*.14,h*.64,w*.38,h*.13,unit*.021,unit*.013,500,p.muted);buttonDraw(w*.14,h*.82,w*.31);
 }else if(model===3){ // Ticket evento
  drawImage(0,0,w,h,.28);ctx.fillStyle=p.surface;rounded(m,h*.18,w-m*2,h*.64,unit*.020);ctx.fill();ctx.strokeStyle=p.line;ctx.setLineDash([unit*.012,unit*.008]);ctx.stroke();ctx.setLineDash([]);badgeDraw(m*1.5,h*.22,w*.28);textBlock('title',title,m*1.5,h*.34,w-m*3,h*.20,unit*.060,unit*.030,900,p.ink,'center');textBlock('subtitle',subtitle,m*2,h*.56,w-m*4,h*.12,unit*.021,unit*.013,500,p.muted,'center');buttonDraw((w-w*.34)/2,h*.72,w*.34);
 }else if(model===4){ // Magazine
  drawImage(m,m,w*.56,h-m*2,0);ctx.fillStyle=p.surface;ctx.fillRect(w*.56,0,w*.44,h);badgeDraw(w*.60,m,w*.30);textBlock('title',title,w*.60,h*.20,w*.34,h*.30,unit*.054,unit*.028,900,p.ink);textBlock('subtitle',subtitle,w*.60,h*.52,w*.32,h*.18,unit*.021,unit*.013,500,p.muted);buttonDraw(w*.60,h*.78,w*.28);
 }else if(model===5){ // Cards
  drawImage(0,0,w,h*.44,.18);badgeDraw(m,m,w*.30);ctx.fillStyle=p.bg;ctx.fillRect(0,h*.40,w,h*.60);textBlock('title',title,m,h*.43,w-m*2,h*.13,unit*.050,unit*.026,900,p.ink);cards(m,h*.58,w-m*2,h*.25,2,4,true);buttonDraw(m,h*.86,w*.32);
 }else if(model===6){ // Minimal
  ctx.fillStyle=p.surface;ctx.fillRect(m,m,w-m*2,h-m*2);ctx.strokeStyle=p.line;ctx.lineWidth=unit*.003;ctx.strokeRect(m,m,w-m*2,h-m*2);drawImage(w*.60,m*1.5,w*.31,h-m*3,0);badgeDraw(m*1.5,m*1.5,w*.28);textBlock('title',title,m*1.5,h*.24,w*.42,h*.30,unit*.058,unit*.028,800,p.ink);textBlock('subtitle',subtitle,m*1.5,h*.57,w*.40,h*.17,unit*.020,unit*.013,500,p.muted);buttonDraw(m*1.5,h*.80,w*.30);
 }else if(model===7){ // Cinematico
  drawImage(0,0,w,h,.50);ctx.fillStyle='rgba(0,0,0,.35)';ctx.fillRect(0,0,w,h);ctx.strokeStyle=p.accent;ctx.lineWidth=unit*.004;ctx.strokeRect(m,m,w-m*2,h-m*2);badgeDraw((w-w*.30)/2,m,w*.30);textBlock('title',title,m,h*.34,w-m*2,h*.25,unit*.073,unit*.034,900,'#fff','center');textBlock('subtitle',subtitle,w*.18,h*.62,w*.64,h*.12,unit*.022,unit*.014,500,'#fff','center');buttonDraw((w-w*.34)/2,h*.80,w*.34);
 }else if(model===8){ // Diagonale dinamico
  drawImage(w*.38,0,w*.62,h,0);ctx.fillStyle=p.bg;ctx.beginPath();ctx.moveTo(0,0);ctx.lineTo(w*.58,0);ctx.lineTo(w*.35,h);ctx.lineTo(0,h);ctx.closePath();ctx.fill();ctx.strokeStyle=p.accent;ctx.lineWidth=unit*.008;ctx.beginPath();ctx.moveTo(w*.58,0);ctx.lineTo(w*.35,h);ctx.stroke();badgeDraw(m,m,w*.30);textBlock('title',title,m,h*.22,w*.36,h*.32,unit*.060,unit*.030,900,p.ink);textBlock('subtitle',subtitle,m,h*.57,w*.31,h*.16,unit*.020,unit*.013,500,p.muted);buttonDraw(m,h*.80,w*.27);
 }else{ // Collage editoriale
  drawImage(m,m,w*.55,h*.48,0);drawImage(w*.62,m,w*.30,h*.22,0);drawImage(w*.62,h*.29,w*.30,h*.23,0);ctx.fillStyle=p.surface;rounded(m,h*.56,w-m*2,h*.35,unit*.016);ctx.fill();badgeDraw(m*1.4,h*.59,w*.28);textBlock('title',title,m*1.4,h*.68,w*.46,h*.15,unit*.050,unit*.026,900,p.ink);buttonDraw(w*.65,h*.73,w*.25);
 }
 // Solo i modelli informativi mostrano quadranti: non più pannello fisso globale.
 if([0,1,2,3,4,6,7,8,9].includes(model) && d.contentMode==='complete' && sections.length){
  if(model===0)cards(w*.52,h*.58,w*.42,h*.27,2,4,true);
  else if(model===1)cards(w*.60,h*.58,w*.34,h*.25,1,2,false);
  else if(model===2)cards(w*.60,h*.60,w*.34,h*.25,1,2,true);
  else if(model===3)cards(m*1.5,h*.58,w-m*3,h*.12,4,4,true);
  else if(model===4)cards(w*.60,h*.59,w*.34,h*.15,1,2,true);
  else if(model===6)cards(m*1.5,h*.62,w*.40,h*.14,2,2,true);
  else if(model===7)cards(m,h*.71,w-m*2,h*.10,4,4,false);
  else if(model===8)cards(w*.60,h*.62,w*.33,h*.24,1,2,true);
  else if(model===9)cards(m*1.4,h*.83,w*.47,h*.06,2,2,true);
 }
 footerDraw();drawLogos();ctx.strokeStyle='rgba(20,30,40,.14)';ctx.lineWidth=Math.max(1,unit*.0015);ctx.strokeRect(1,1,w-2,h-2);return canvas;
}
