...
function normalizeOffsets(value){const out=Object.fromEntries(OFFSET_KEYS.map(k=>[k,{x:0,y:0}]));if(value&&typeof value==="object")OFFSET_KEYS.forEach(k=>{if(value[k]){out[k].x=Number(value[k].x)||0;out[k].y=Number(value[k].y)||0}});return out}function normalizeScales(value){const out=Object.fromEntries(SCALE_KEYS.map(k=>[k,100]));if(value&&typeof value==="object")SCALE_KEYS.forEach(k=>{if(value[k]!=null){const min=k==="image"?100:40,max=k==="image"?300:200;out[k]=Math.max(min,Math.min(max,Number(value[k])||100))}});return out}
function scaleSnapshot(){return JSON.parse(JSON.stringify(elementScales))}
function elementScale(key){const min=key==="image"?1:.4,max=key==="image"?3:2;return Math.max(min,Math.min(max,Number(elementScales[key]||100)/100))}
function updateSelectedScaleControl(){const key=$("moveTarget")?.value||"title",value=Math.round(elementScales[key]||100),names={image:"Immagine principale",title:"Titolo",subtitle:"Sottotitolo / slogan",details:"Descrizione e pannello",cta:"CTA / pulsante",badge:"Badge superiore",footer:"Data e luogo",logo:"Logo 1",logo2:"Logo 2",logo3:"Logo 3",logo4:"Logo 4",logo5:"Logo 5",logo6:"Logo 6",logo7:"Logo 7",logo8:"Logo 8"};if($("selectedElementScale")){const slider=$("selectedElementScale");slider.min=key==="image"?100:40;slider.max=key==="image"?300:200;slider.value=value;}if($("selectedElementScaleValue"))$("selectedElementScaleValue").textContent=`Dimensione: ${value}%`;if($("selectedElementName"))$("selectedElementName").textContent=names[key]||key}
...
function editableMainImage(source,d){
 if(!source)return null;
 const sw=source.naturalWidth||source.width,sh=source.naturalHeight||source.height;if(!sw||!sh)return source;
 const c=document.createElement("canvas");c.width=sw;c.height=sh;const x=c.getContext("2d",{alpha:false});x.imageSmoothingEnabled=true;x.imageSmoothingQuality="high";
 const scale=elementScale("image"),dw=sw*scale,dh=sh*scale,o=(d.manualOffsets||manualOffsets)?.image||{x:0,y:0},factor=Math.min(sw,sh)/1080;
 let dx=(sw-dw)/2+(Number(o.x)||0)*factor,dy=(sh-dh)/2+(Number(o.y)||0)*factor;
 dx=clamp(dx,sw-dw,0);dy=clamp(dy,sh-dh,0);
 x.fillStyle="#000";x.fillRect(0,0,sw,sh);x.drawImage(source,dx,dy,dw,dh);return c;
}
...
async function renderDesign(canvas,d){
 ...
 const [rawImg,logo,logo2,...extraLogoImages]=await Promise.all([loadImg(currentImage),loadImg(currentLogo),loadImg(currentLogo2),...extraLogos.map(l=>loadImg(l.data))]);
 const img=editableMainImage(rawImg,d);
 canvas._elementBoxes={};if(img)canvas._elementBoxes.image={x:0,y:0,w,h};
...
function flushForegroundLogo(){
 ...
 function richInfoPanel(){
  ...
  const panelTypographyScale=clamp(elementScale("details"),.92,1.18);
  ...
  let availableSections=[
    benefits.length?{title:sectionLabels[0],items:benefits,priority:1}:null,
    program.length?{title:sectionLabels[1],items:program,priority:2}:null,
    targets.length?{title:sectionLabels[2],items:targets,priority:3}:null,
    method.length?{title:"Metodo di lavoro",items:method,priority:4}:null,
    included.length?{title:"Cosa è incluso",items:included,priority:5}:null,
    requirements.length?{title:"Requisiti / materiale",items:requirements,priority:6}:null,
    notes.length?{title:"Note importanti",items:notes,priority:7}:null,
    info.length?{title:sectionLabels[3],items:info,priority:8}:null
  ].filter(Boolean);

  // Una locandina non deve diventare un documento: preserviamo leggibilità e
  // completezza mostrando un numero di blocchi compatibile con il formato.
  const sectionLimit=portrait?3:(ultra?4:4);
  if(d.contentMode==="balanced")availableSections=availableSections.slice(0,Math.min(4,sectionLimit));
  else if(d.contentMode==="essential")availableSections=availableSections.slice(0,2);
  else availableSections=availableSections.slice(0,sectionLimit);
  if(!availableSections.length)return;

  const safeInset=Math.max(m,unit*.028);
  const px=safeInset,pw=w-safeInset*2;
  const totalChars=availableSections.reduce((sum,e)=>sum+e.title.length+e.items.join(" ").length,0);

  // Il pannello guadagna altezza quando il testo è più denso, evitando che
  // titoli, paragrafi e CTA finiscano uno sopra l'altro.
  const densityLift=clamp((totalChars-280)/1500,0,.10);
  const targetTop=portrait?h*(.49-densityLift):h*(.47-densityLift);
  const preferredTop=Math.max(targetTop,Math.min(contentBottom+unit*.018,h*.57));
  const minPanelH=portrait?h*.43:(ultra?h*.43:h*.45);
  const latestTop=h-safeInset-minPanelH;
  const py=clamp(preferredTop,safeInset,latestTop);
  const ph=h-py-safeInset;
  if(ph<unit*.32)return;

  ctx.save();ctx.globalCompositeOperation="source-over";
  const grad=ctx.createLinearGradient(0,py,0,h);
  if(light){grad.addColorStop(0,"rgba(250,249,246,.98)");grad.addColorStop(1,"rgba(230,235,238,.995)")}
  else{grad.addColorStop(0,"rgba(4,8,13,.96)");grad.addColorStop(1,"rgba(2,5,9,.995)")}
  ctx.fillStyle=grad;
  const radius=unit*.020;
  roundedRect(ctx,px,py,pw,ph,radius);ctx.fill();registerElementBox("details",px,py,pw,ph,"left");
  ctx.strokeStyle=accent;ctx.lineWidth=Math.max(2,unit*.0024);ctx.stroke();

  const pad=unit*(portrait?.026:.023),gap=unit*(portrait?.015:.016);
  // Nei formati orizzontali quattro sezioni diventano una griglia 2×2:
  // niente più quattro colonne strette con caratteri minuscoli.
  let cols=portrait?1:2;
  if(availableSections.length<=2&&!portrait)cols=availableSections.length;
  const rows=Math.ceil(availableSections.length/cols);
  const actionH=clamp(unit*.064*typeBoost,unit*.052,unit*.078);
  const actionTop=py+ph-actionH-unit*.016;
  const gridTop=py+pad;
  const gridBottom=actionTop-unit*.018;
  const rowGap=gap;
  const rowH=(gridBottom-gridTop-rowGap*(rows-1))/rows;
  const colW=(pw-pad*2-gap*(cols-1))/cols;

  const headingBase=clamp(unit*(portrait?.022:.020)*typeBoost*panelTypographyScale,17*scale,39*scale);
  const bodyBase=clamp(unit*(portrait?.0185:.0175)*typeBoost*panelTypographyScale,13*scale,31*scale);

  function prepareSection(title,items,maxW,maxH){
    let heading=headingBase,body=bodyBase;
    const titleText=title.toUpperCase();
    ctx.font=`900 ${heading}px Arial`;
    while(heading>13*scale&&ctx.measureText(titleText).width>maxW){heading-=1;ctx.font=`900 ${heading}px Arial`;}
    let lines=[];
    while(body>=10*scale){
      ctx.font=`600 ${body}px Arial`;
      const lh=body*1.22;
      lines=items.map(item=>wrapAllLines(ctx,item,maxW-body*.95));
      const needed=heading*1.42+lines.reduce((sum,row)=>sum+row.length*lh+body*.22,0);
      if(needed<=maxH)break;
      body-=1;
    }
    return {titleText,heading,body,lines,lineH:body*1.22};
  }
  ...
  availableSections.forEach((entry,index)=>{
    ...
    const cardH=rowH;
    ctx.save();
    ctx.fillStyle=light?"rgba(255,255,255,.72)":"rgba(255,255,255,.055)";
    roundedRect(ctx,sx,sy,colW,cardH,unit*.012);ctx.fill();
    ctx.strokeStyle=light?"rgba(36,58,74,.16)":"rgba(255,255,255,.10)";
    ctx.lineWidth=Math.max(1,unit*.0012);ctx.stroke();

    const inner=unit*.014;
    const fit=prepareSection(entry.title,entry.items,colW-inner*2,cardH-inner*2);
    let yy=sy+inner;
    ctx.fillStyle=accent;ctx.textAlign="left";ctx.textBaseline="top";
    ctx.font=`900 ${fit.heading}px Arial`;ctx.fillText(fit.titleText,sx+inner,yy);
    yy+=fit.heading*1.38;
    ctx.font=`600 ${fit.body}px Arial`;
    entry.items.forEach((item,i)=>{
      const itemLines=fit.lines[i]||[];
      ctx.fillStyle=i%2?gold:accent;
      ctx.beginPath();ctx.arc(sx+inner+fit.body*.22,yy+fit.body*.42,Math.max(1.4,fit.body*.16),0,Math.PI*2);ctx.fill();
      ctx.fillStyle=light?"#18212a":"#f4f6f8";
      itemLines.forEach(line=>{ctx.fillText(line,sx+inner+fit.body*.62,yy);yy+=fit.lineH;});
      yy+=fit.body*.20;
    });
    ctx.restore();
  });
  ...
  const ctaW=portrait?pw*.70:(ultra?pw*.34:pw*.40);
...
function readImage(input,kind){
  ...
  r.onload=async()=>{
    try{
      ...
      if(kind==="image"){
        currentImage=r.result;manualOffsets.image={x:0,y:0};elementScales.image=100;selectedCanvasElement="image";if($("moveTarget"))$("moveTarget").value="image";updateOffsetReadout();updateSelectedScaleControl();
        notify("Immagine caricata: ora puoi selezionarla, spostarla e ingrandirla direttamente nell’anteprima.");
      }else if(kind==="logo2"){
...
$("selectedElementScale").addEventListener("input",()=>{const key=$("moveTarget").value;const min=key==="image"?100:40,max=key==="image"?300:200;elementScales[key]=Math.max(min,Math.min(max,Number($("selectedElementScale").value)||100));updateSelectedScaleControl();build(false)});
function changeSelectedElementScale(delta){const key=$("moveTarget").value,min=key==="image"?100:40,max=key==="image"?300:200;elementScales[key]=Math.max(min,Math.min(max,(elementScales[key]||100)+delta));updateSelectedScaleControl();build(false)}
...
function hitCanvasElement(canvas,p){
 ...
 const priority={logo:1,logo2:1,logo3:1,logo4:1,logo5:1,logo6:1,logo7:1,logo8:1,cta:2,title:3,subtitle:4,badge:5,footer:6,details:7,image:99};
...
function beginCanvasDrag(event){
 ...
 selectedCanvasElement=key;activeLogoKey=key.startsWith("logo")?key:activeLogoKey;canvas.style.cursor="grabbing";
 if($("moveTarget"))$("moveTarget").value=key;
 updateOffsetReadout();updateSelectedScaleControl();
...
function endCanvasDrag(event){
 ...
 activeCanvasDrag=null;$("previewCanvas").style.cursor="grab";
