...
async function renderDesign(canvas,d){
 ...
 function cleanPosterLines(value,maxItems=3,maxChars=120){
  ...
  String(value||"").split(/\n|•|\u2022|;/).map(v=>v.replace(/^[-–—\s]+/,"").replace(/\s+/g," ").trim()).filter(Boolean).forEach(line=>{
   ...
   seen.add(key);
   out.push(line.length>maxChars?line.slice(0,maxChars-1).trimEnd()+"…":line);
  });
  return out;
 }
 function sectionValue(value,maxItems=3,maxChars=120){return cleanPosterLines(value,maxItems,maxChars).join(" · ")}
 const audienceValue=hasText(d.targetText)?d.targetText:(hasText(d.audience)&&!/^tutti$/i.test(String(d.audience).trim())?d.audience:"");
 const rawSections=[
  ["DESCRIZIONE",sectionValue(d.details,2,145)],
  ["VANTAGGI",sectionValue(d.benefits,3,92)],
  ["PROGRAMMA",sectionValue(d.program,3,92)],
  ["PER CHI È INDICATO",sectionValue(audienceValue,2,110)],
  ["METODO",sectionValue(d.methodText,2,110)],
  ["INCLUSO",sectionValue(d.includedText,3,88)],
  ["REQUISITI",sectionValue(d.requirementsText,2,100)],
  ["NOTE",sectionValue(d.notesText,2,100)],
  ["INFO EVENTO",sectionValue([d.eventTime,d.eventSeats,d.eventPrice,d.eventPhone].filter(hasText).join(" · "),3,90)],
  ["CONTATTI",sectionValue(d.contact,2,100)]
 ...
 function cardGrid(x,y,bw,bh,cols=2,limit=4,mode="cards"){
  ...
  // La locandina non è una scheda tecnica: mostra pochi contenuti, realmente leggibili.
  ...
  const gap=Math.max(10,unit*.012),rows=Math.ceil(items.length/cols),cw=(bw-gap*(cols-1))/cols,ch=(bh-gap*(rows-1))/rows;
  items.forEach(([head,body],i)=>{const cx=x+o.x+(i%cols)*(cw+gap),cy=y+o.y+Math.floor(i/cols)*(ch+gap);ctx.save();
   ...
   else{ctx.fillStyle=p.surface;ctx.globalAlpha=.96;rr(cx,cy,cw,ch,unit*.009);ctx.fill();ctx.globalAlpha=1;ctx.strokeStyle=p.accent;ctx.lineWidth=Math.max(3,unit*.004);ctx.beginPath();ctx.moveTo(cx,cy);ctx.lineTo(cx,cy+ch);ctx.stroke()}
   const pad=Math.max(12,unit*.015),headSize=Math.max(14,unit*.020);
   ctx.fillStyle=p.accent;ctx.font=`900 ${headSize}px Arial`;ctx.textAlign="left";ctx.textBaseline="top";ctx.fillText(fitEllipsis(ctx,head,cw-pad*2),cx+pad,cy+pad);
   const bodyText=String(body||"");
   const maxBody=Math.max(18,unit*(ratio>1.2?.029:.031));
   const minBody=Math.max(17,unit*(ratio>1.2?.024:.026));
   const bodyTop=cy+pad+headSize*1.35,bodyHeight=Math.max(0,ch-(bodyTop-cy)-pad);
   const r=fitBlock(bodyText,cw-pad*2,bodyHeight,maxBody,minBody,600);
   ctx.fillStyle=p.ink;ctx.font=`600 ${r.size}px Arial`;r.lines.forEach((line,j)=>ctx.fillText(line,cx+pad,bodyTop+j*r.size*1.18));ctx.restore();
  });
 }
 function footer(){
 ...
 if(model===0){ // Hero fotografico
  ...
  if(rawSections.length)cardGrid(m,h*.57,w-m*2,h*.27,2,4,"cards");button(m,h*.87,w*.34);
 }
