...
function generatedFieldValues(service,key){
 const p=serviceProfile(service),cls=formatClass($("type")?.value||"Post Instagram quadrato");
...
async function renderDesign(canvas,d){
 const f=formats[d.type]||formats["Locandina A4 verticale"],w=f.w,h=f.h,ctx=canvas.getContext("2d"),unit=Math.min(w,h);
 ...
 const audienceValue=hasText(d.targetText)?d.targetText:"";
 const rawSections=[
 ...
 function fitBlock(text,bw,bh,max,min,weight=800){
  ...
  let size=Math.max(min,max),lines=[];
  const absoluteMin=Math.max(10,Math.min(min,unit*.012));
  for(;size>=absoluteMin;size-=1){
   ctx.font=`${weight} ${size}px Arial,sans-serif`;
   lines=wrapAllLines(ctx,value,bw);
   if(lines.length*size*1.10<=bh)break;
  }
  // Il testo non viene mai alterato o abbreviato. Se il contenuto eccede,
  // il box chiamante deve riservare più spazio o mostrare meno sezioni.
  return{size,lines,lineHeight:size*1.10,height:lines.length*size*1.10};
 }
 ...
 function cardGrid(x,y,bw,bh,cols=2,limit=4,mode="cards"){
  const o=off("details"),ratio=w/h,cls=formatClass(d.type);
  const formatLimit={ultraWide:2,landscape:3,printLandscape:5,square:4,portrait:5,printPortrait:8}[cls]||4;
  const modeLimit=d.contentMode==="essential"?Math.min(3,formatLimit):d.contentMode==="compact"?Math.min(5,formatLimit):formatLimit;
  const requested=Math.max(1,Math.min(rawSections.length,Number(limit)||modeLimit,modeLimit));
  const items=rawSections.slice(0,requested);
  if(!items.length)return 0;

  cols=ratio<.9?1:Math.min(Math.max(1,cols),items.length,2);
  const gap=Math.max(6,unit*.007),pad=Math.max(9,unit*.010),cw=(bw-gap*(cols-1))/cols;
  const minBody=Math.max(12,unit*(cls==="ultraWide"?.012:cls==="landscape"?.0135:.015));
  const startBody=Math.max(minBody,unit*(cls==="ultraWide"?.016:cls==="landscape"?.0175:.020));

  function calculate(size){
   const headSize=Math.max(11,size*.76),lineHeight=size*1.18;
   const lines=items.map(([,body])=>{ctx.font=`600 ${size}px Arial,sans-serif`;return wrapAllLines(ctx,String(body||""),cw-pad*2)});
   const heights=lines.map(ls=>pad+headSize*1.22+Math.max(1,ls.length)*lineHeight+pad);
   const rowHeights=[];
   for(let r=0;r<Math.ceil(items.length/cols);r++){
    let rh=0;for(let c=0;c<cols;c++){const i=r*cols+c;if(i<heights.length)rh=Math.max(rh,heights[i])}
    rowHeights.push(rh);
   }
   return{headSize,lineHeight,lines,rowHeights,total:rowHeights.reduce((a,b)=>a+b,0)+gap*(rowHeights.length-1)};
  }

  let bodySize=startBody,layout=calculate(bodySize);
  while(layout.total>bh&&bodySize>minBody){bodySize-=.5;layout=calculate(bodySize)}

  // Se il contenuto richiede più spazio, il pannello cresce verso l'alto.
  // Nessuna frase viene troncata e non vengono aggiunti puntini di sospensione.
  const availableAbove=Math.max(0,y-m);
  const extra=Math.min(Math.max(0,layout.total-bh),availableAbove);
  const drawY=y+o.y-extra,drawH=Math.max(bh,layout.total);
  box("details",x+o.x,drawY,bw,Math.min(drawH,bh+availableAbove));

  let rowHeights=layout.rowHeights.slice();
  const usable=Math.max(layout.total,bh+extra);
  if(layout.total<usable){const add=(usable-layout.total)/rowHeights.length;rowHeights=rowHeights.map(v=>v+add)}

  items.forEach(([head],i)=>{
   const row=Math.floor(i/cols),col=i%cols;
   const cy=drawY+rowHeights.slice(0,row).reduce((a,b)=>a+b,0)+gap*row;
   const cx=x+o.x+col*(cw+gap),ch=rowHeights[row];
   ctx.save();
   ctx.fillStyle=p.surface;ctx.globalAlpha=mode==="cards"?.98:.96;rr(cx,cy,cw,ch,unit*.009);ctx.fill();ctx.globalAlpha=1;
   ctx.strokeStyle=mode==="cards"?p.line:p.accent;ctx.lineWidth=Math.max(1,unit*(mode==="cards"?.0012:.0025));ctx.stroke();
   ...
   ctx.fillStyle=p.ink;ctx.font=`600 ${bodySize}px Arial,sans-serif`;
   const top=cy+pad+layout.headSize*1.22;
   layout.lines[i].forEach((line,j)=>ctx.fillText(line,cx+pad,top+j*layout.lineHeight));
   ctx.restore();
  });
  return Math.min(drawH,bh+availableAbove);
 }
 function footer(){
 ...
 if(model===0){ // Hero fotografico
  ...
  badgeDraw(m,m,Math.min(w*.36,unit*.38));
  text("title",title,m,hasBlocks?h*.34:h*.48,w-m*2,hasBlocks?h*.16:h*.22,unit*.068,unit*.028,900,"#fff");
  text("subtitle",subtitle,m,hasBlocks?h*.51:h*.70,w*.72,hasBlocks?h*.07:h*.10,unit*.022,unit*.013,500,"#fff");
  if(hasBlocks)cardGrid(m,h*.59,w-m*2,h*.27,portrait?1:2,portrait?5:4,"lines");
  button(m,hasBlocks?h*.88:h*.84,Math.min(w*.34,unit*.42));decorations("cinematic");
  ...
  if(hasBlocks)cardGrid(w*.08,h*.59,w*.84,h*.27,portrait?1:2,portrait?5:3,"lines");
  button((w-w*.36)/2,hasBlocks?h*.88:h*.80,w*.36);
 ...
 if(rawSections.length&&!canvas._elementBoxes.details){
  const fallbackCount=d.contentMode==="complete"?rawSections.length:(ratio>2?3:ratio>1.2?4:ratio<.82?5:4);
  const oy=portrait?h*.58:(landscape?h*.60:h*.62),oh=portrait?h*.31:(landscape?h*.27:h*.25);
  ctx.save();ctx.fillStyle=p.surface;ctx.globalAlpha=.96;rr(m,oy,w-m*2,oh,unit*.010);ctx.fill();ctx.globalAlpha=1;ctx.restore();
  const cols=portrait?1:Math.min(2,fallbackCount);
  cardGrid(m,oy,w-m*2,oh,cols,fallbackCount,"lines");
 }
...
function importProject(file){const r=new FileReader();r.onload=()=>{try{const d=JSON.parse(r.result);["project","type","externalTextPosition","externalTextPanel","discipline","objective","audience","tone","tone2","toneMix","style","styleMode","logoSize","logoScale","logo2Size","logo2Scale","logo2Position","logoBgMode","logoTolerance","logo2BgMode","logo2Tolerance","contentMode","sloganInput","subtitleInput","details","benefits","program","targetText","methodText","includedText","requirementsText","notesText","eventTime","eventSeats","eventPrice","eventPhone","contact","ctaType","ctaValue","ctaMessage","date","location"].forEach(k=>{if(d[k]!=null&&$(k))$(k).value=d[k]});$("type").value=d.type==="Locandina A4 orizzontale"?"Locandina A4 orizzontale":"Locandina A4 verticale";$("titleInput").value=d.title||"";if($("externalPosterMode"))$("externalPosterMode").checked=d.externalPosterMode===true;$("sloganInput").value=d.slogan||"";$("subtitleInput").value=d.subtitle||"";$("accentColor").value=d.accent||"#ff7a00";$("goldColor").value=d.gold||"#d8ab4d";currentImage=d.image||"";currentLogo=d.logo||"";currentLogoOriginal=d.logoOriginal||d.logo||"";currentLogo2=d.logo2||"";currentLogo2Original=d.logo2Original||d.logo2||"";extraLogos=Array.isArray(d.extraLogos)?d.extraLogos:[];if($("ctaPdfEnabled"))$("ctaPdfEnabled").checked=d.ctaPdfEnabled!==false;if($("ctaSocialEnabled"))$("ctaSocialEnabled").checked=d.ctaSocialEnabled!==false;updateCtaControls();if($("logoBgMode")&&d.logoBgMode)$("logoBgMode").value=d.logoBgMode;if($("logoScale")&&d.logoScale!=null){$("logoScale").value=d.logoScale;$("logoScaleValue").textContent=d.logoScale+"%"}if($("logo2Scale")&&d.logo2Scale!=null){$("logo2Scale").value=d.logo2Scale;$("logo2ScaleValue").textContent=d.logo2Scale+"%"}if($("logoTolerance")&&d.logoTolerance!=null){$("logoTolerance").value=d.logoTolerance;$("logoToleranceValue").textContent=d.logoTolerance}if($("logo2Tolerance")&&d.logo2Tolerance!=null){$("logo2Tolerance").value=d.logo2Tolerance;$("logo2ToleranceValue").textContent=d.logo2Tolerance}graphicVariant=normalizeGraphicVariant($("type").value,d.graphicVariant??0);manualOffsets=normalizeOffsets(d.manualOffsets);elementScales=normalizeScales(d.elementScales);updatePreviewGraphicStatus();updateSelectedScaleControl();applyMedia();updateOffsetReadout();build();notify("Progetto importato.")}catch{notify("File JSON non valido.",true)}};r.readAsText(file)}
...
function bindReliableCommands(){
 ...
 const textPanel=document.querySelector('.tab-panel[data-panel="contents"]');
 if(textPanel){
