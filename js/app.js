...
const $=id=>document.getElementById(id), STORAGE_KEY="k9CreativeStudioV220AdaptiveMultiLogo";
...
const formats={
"Locandina A4 verticale":{label:"A4 verticale · 2480 × 3508 px · 300 DPI",w:2480,h:3508},
"Locandina A4 orizzontale":{label:"A4 orizzontale · 3508 × 2480 px · 300 DPI",w:3508,h:2480}
};
function normalizeFormat(value){return value==="Locandina A4 orizzontale"?value:"Locandina A4 verticale"}
...
function expandProfileLine(line){
 const clean=String(line||"").replace(/\s+/g," ").trim().replace(/[.;]+$/,'');
 if(!clean)return "";
 // Mantiene le frasi sintetiche: evita l'aggiunta automatica di formule ripetitive.
...
function compactGeneratedLine(value,maxChars=118){
 // Mantiene integralmente il contenuto: nessun taglio, nessun puntino di sospensione.
 // La leggibilità viene gestita dal motore di impaginazione, non alterando il testo.
 const text=String(value||"").replace(/\s+/g," ").trim();
 return text.replace(/[.;]+$/,'');
}
function uniqueGeneratedLines(values,maxItems,maxChars){
 const out=[],seen=new Set();
 for(const value of values||[]){
  const line=compactGeneratedLine(value,maxChars),key=line.toLocaleLowerCase("it-IT").replace(/[^a-z0-9à-ÿ]+/g," ").trim();
...
function generatedFieldValues(service,key){
 const p=serviceProfile(service),cls=formatClass($("type")?.value||"Locandina A4 verticale");
 const limits={portrait:5,square:4,landscape:3,ultraWide:2,printPortrait:6,printLandscape:4};
 const maxItems=limits[cls]||4,maxChars=(cls==="ultraWide"||cls==="landscape")?92:118;
 const values={
 ...
 return uniqueGeneratedLines(values[key]||[],["method","notes"].includes(key)?1:maxItems,maxChars);
}
...
function formatClass(type){const f=formats[type]||formats["Locandina A4 verticale"],r=f.w/f.h;if(/A4|A5|Brochure|Attestato/.test(type))return r<1?"printPortrait":"printLandscape";if(r<.82)return"portrait";if(r>2)return"ultraWide";if(r>1.25)return"landscape";return"square"}
const FORMAT_COLOR_THEMES={
 portrait:["Chiaro professionale","Natura salvia","Energia corallo","Premium grafite","Tattico verde","Editoriale viola","Chiaro sabbia","Premium nero oro"],
 square:["Premium nero oro","Energia arancio","Tattico blu","Editoriale viola","Chiaro professionale","Natura organica","Premium grafite","Editoriale petrolio"],
 landscape:["Chiaro sabbia","Premium grafite","Tattico blu","Editoriale petrolio","Energia arancio","Natura salvia","Premium nero oro","Chiaro professionale"],
 ultraWide:["Tattico blu","Premium grafite","Editoriale petrolio","Energia corallo","Chiaro professionale","Natura organica"],
 printPortrait:["Chiaro professionale","Chiaro sabbia","Natura salvia","Editoriale viola","Premium grafite","Energia arancio","Tattico verde","Premium nero oro"],
 printLandscape:["Chiaro professionale","Premium nero oro","Premium grafite","Editoriale petrolio","Tattico blu","Chiaro sabbia","Natura organica","Energia corallo"]
};
function compatibleThemes(type){
 const wanted=FORMAT_COLOR_THEMES[formatClass(type)]||FORMAT_COLOR_THEMES.square;
 const all=[...$("style").options].map(o=>o.value||o.text).filter(Boolean);
 return wanted.filter(name=>all.includes(name));
}
function applyFormatAdaptiveScale(type,changeTheme=true){
 const f=formats[type]||formats["Locandina A4 verticale"],cls=formatClass(type),ratio=f.w/f.h;
 const profiles={
 ...
 if($("formatAdaptStatus"))$("formatAdaptStatus").textContent=`Adattamento completo ${f.w} × ${f.h}px · testi ${ratio>2?"panoramici":ratio<.75?"verticali":"bilanciati"} · 12 modelli reali e ${compatibleThemes(type).length} temi dedicati.`;
}
...
const DEFAULT_TEXT_COLORS={title:"",subtitle:"",details:"",badge:"",cta:"",footer:""};
let textColors={...DEFAULT_TEXT_COLORS};
const PALETTE_STORAGE_KEY="k9_custom_palettes_v1";
const BUILTIN_PALETTES={
 professional:{background:"#f7fbff",surface:"#ffffff",ink:"#10263a",muted:"#52687a",accent:"#176b87",secondary:"#d8ab4d"},
 tactical:{background:"#07121b",surface:"#0d1b27",ink:"#eef5f8",muted:"#a6bac5",accent:"#ff7a00",secondary:"#1f6d83"},
 nature:{background:"#edf7f0",surface:"#ffffff",ink:"#173629",muted:"#587064",accent:"#4f9b67",secondary:"#d99b42"},
 premium:{background:"#0b1118",surface:"#111a24",ink:"#f8f4e8",muted:"#c3bda9",accent:"#d7a84a",secondary:"#7a5b25"},
 light:{background:"#ffffff",surface:"#f4f7f9",ink:"#14212b",muted:"#5f6d78",accent:"#247ba0",secondary:"#f28c28"}
};
function getCustomPalettes(){try{return JSON.parse(localStorage.getItem(PALETTE_STORAGE_KEY)||"[]")||[]}catch{return[]}}
function setCustomPalettes(v){localStorage.setItem(PALETTE_STORAGE_KEY,JSON.stringify(v));refreshSavedPaletteSelect()}
function currentPaletteValues(){return{background:$("paletteBackground")?.value||"#f7fbff",surface:$("paletteSurface")?.value||"#ffffff",ink:$("paletteInk")?.value||"#10263a",muted:$("paletteMuted")?.value||"#52687a",accent:$("accentColor")?.value||"#ff7a00",secondary:$("goldColor")?.value||"#d8ab4d"}}
function applyPaletteValues(p,render=true){if(!p)return;const map={paletteBackground:"background",paletteSurface:"surface",paletteInk:"ink",paletteMuted:"muted",accentColor:"accent",goldColor:"secondary"};Object.entries(map).forEach(([id,key])=>{if($(id)&&p[key])$(id).value=p[key]});if(render)build(false)}
function refreshSavedPaletteSelect(){const el=$("savedPaletteSelect");if(!el)return;const selected=el.value;el.innerHTML='<option value="">Palette salvate</option>'+getCustomPalettes().map((p,i)=>`<option value="${i}">${esc(p.name)}</option>`).join('');if([...el.options].some(o=>o.value===selected))el.value=selected}
function selectedTextColorValue(key){return textColors?.[key]||""}
function setSelectedTextColor(key,value){if(!Object.prototype.hasOwnProperty.call(DEFAULT_TEXT_COLORS,key))return;textColors={...DEFAULT_TEXT_COLORS,...textColors,[key]:value};if($("selectedTextColor"))$("selectedTextColor").value=value||"#ffffff"}
function projectData(autoTitle=false){const project=$("project").value.trim()||"Progetto K9",type=normalizeFormat($("type").value),externalPosterMode=$("externalPosterMode")?.checked===true,externalTextPosition=$("externalTextPosition")?.value||"auto",externalTextPanel=$("externalTextPanel")?.value||"soft",externalPanelIntensity=Number($("externalPanelIntensity")?.value??75),externalPanelOpacity=Number($("externalPanelOpacity")?.value??85),discipline=$("discipline").value,objective=$("objective").value,audience=$("audience").value,tone=$("tone").value,tone2=$("tone2").value,toneMix=Number($("toneMix").value||70),style=$("style").value,styleMode=$("styleMode")?.value||"auto",logoSize=$("logoSize").value||"auto",logoScale=Number($("logoScale")?.value||100),logo2Size=$("logo2Size")?.value||"auto",logo2Scale=Number($("logo2Scale")?.value||100),logo2Position=$("logo2Position")?.value||"tl",contentMode=$("contentMode")?.value||"complete",d=disciplines[discipline],details=$("details").value.trim(),subtitle=$("subtitleInput").value.trim(),slogan=$("sloganInput").value.trim(),contact=$("contact").value.trim(),location=$("location").value.trim(),benefits=$("benefits")?.value.trim()||"",program=$("program")?.value.trim()||"",targetText=$("targetText")?.value.trim()||"",methodText=$("methodText")?.value.trim()||"",includedText=$("includedText")?.value.trim()||"",requirementsText=$("requirementsText")?.value.trim()||"",notesText=$("notesText")?.value.trim()||"",eventTime=$("eventTime")?.value.trim()||"",eventSeats=$("eventSeats")?.value.trim()||"",eventPrice=$("eventPrice")?.value.trim()||"",eventPhone=$("eventPhone")?.value.trim()||"",ctaType=$("ctaType")?.value||"none",ctaValue=$("ctaValue")?.value.trim()||"",ctaMessage=$("ctaMessage")?.value.trim()||"",ctaPdfEnabled=$("ctaPdfEnabled")?.checked!==false,ctaSocialEnabled=$("ctaSocialEnabled")?.checked!==false;if(autoTitle)$("titleInput").value=randomItem(poolFor(discipline,"titles"));return{id:Date.now(),updatedAt:new Date().toISOString(),project,type,externalPosterMode,externalTextPosition,externalTextPanel,externalPanelIntensity,externalPanelOpacity,discipline,objective,audience,tone,tone2,toneMix,style,styleMode,logoSize,logoScale,logo2Size,logo2Scale,logo2Position,logoBgMode:$("logoBgMode")?.value||"auto",logoTolerance:Number($("logoTolerance")?.value||42),logo2BgMode:$("logo2BgMode")?.value||"auto",logo2Tolerance:Number($("logo2Tolerance")?.value||42),contentMode,graphicVariant,manualOffsets:offsetSnapshot(),elementScales:scaleSnapshot(),title:$("titleInput").value.trim(),slogan,subtitle,details,benefits,program,targetText,methodText,includedText,requirementsText,notesText,eventTime,eventSeats,eventPrice,eventPhone,contact,ctaType,ctaValue,ctaMessage,ctaPdfEnabled,ctaSocialEnabled,date:$("date").value,location,accent:$("accentColor").value,gold:$("goldColor").value,palette:currentPaletteValues(),textColors:{...textColors},image:currentImage,logo:currentLogo,logoOriginal:currentLogoOriginal,logo2:currentLogo2,logo2Original:currentLogo2Original,extraLogos:JSON.parse(JSON.stringify(extraLogos))}}
...
async function renderDesign(canvas,d){
 const f=formats[normalizeFormat(d.type)]||formats["Locandina A4 verticale"],w=f.w,h=f.h,ctx=canvas.getContext("2d"),unit=Math.min(w,h);
 ...
 function cleanPosterLines(value,maxItems=99){
  const seen=new Set(),out=[];
  String(value||"").split(/\n|•|\u2022|;/).map(v=>v.replace(/^[-–—\s]+/,"").replace(/\s+/g," ").trim()).filter(Boolean).forEach(line=>{
 ...
 function sectionValue(value,maxItems=99){return cleanPosterLines(value,maxItems).join("\n")}
 const audienceValue=hasText(d.targetText)?d.targetText:(hasText(d.audience)&&!/^tutti$/i.test(String(d.audience).trim())?d.audience:"");
 const rawSections=[
  ["DESCRIZIONE",sectionValue(d.details)],
  ["VANTAGGI",sectionValue(d.benefits)],
  ["PROGRAMMA",sectionValue(d.program)],
  ["PER CHI È INDICATO",sectionValue(audienceValue)],
  ["METODO",sectionValue(d.methodText)],
  ["INCLUSO",sectionValue(d.includedText)],
  ["REQUISITI",sectionValue(d.requirementsText)],
  ["NOTE",sectionValue(d.notesText)],
  ["INFO EVENTO",sectionValue([d.eventTime,d.eventSeats,d.eventPrice,d.eventPhone].filter(hasText).join(" · "))],
  ["CONTATTI",sectionValue(d.contact)]
 ...
 if(d.palette){p={...p,bg:d.palette.background||p.bg,surface:d.palette.surface||p.surface,ink:d.palette.ink||p.ink,muted:d.palette.muted||p.muted,accent:d.palette.accent||p.accent,accent2:d.palette.secondary||p.accent2}}
 const tc={...DEFAULT_TEXT_COLORS,...(d.textColors||{})};
 ...
 function text(k,text,x,y,bw,bh,max,min,weight,color,align="left"){
  ...
  box(k,x+o.x,y+o.y,bw,bh);ctx.fillStyle=tc[k]||color;ctx.textAlign=align;ctx.textBaseline="top";r.lines.forEach((line,i)=>ctx.fillText(line,tx,y+o.y+i*r.size*1.08));return r;
 }
 function badgeDraw(x,y,bw,outline=false){
  const o=off("badge"),hh=unit*.042*scale("badge");box("badge",x+o.x,y+o.y,bw,hh);ctx.save();
  if(outline){ctx.strokeStyle=p.accent;ctx.lineWidth=Math.max(2,unit*.002);rr(x+o.x,y+o.y,bw,hh,hh/2);ctx.stroke();ctx.fillStyle=tc.badge||p.accent}
  else{ctx.fillStyle=p.accent;rr(x+o.x,y+o.y,bw,hh,hh/2);ctx.fill();ctx.fillStyle=tc.badge||p.onAccent}
 ...
 function button(x,y,bw,ghost=false){
  ...
  if(ghost){ctx.strokeStyle=p.accent;ctx.lineWidth=Math.max(2,unit*.002);rr(x+o.x,y+o.y,bw,hh,unit*.012);ctx.stroke();ctx.fillStyle=tc.badge||p.accent}
  else{ctx.fillStyle=p.accent;rr(x+o.x,y+o.y,bw,hh,unit*.012);ctx.fill();ctx.fillStyle=tc.cta||p.onAccent}
 ...
 function cardGrid(x,y,bw,bh,cols=2,limit=4,mode="cards"){
  const o=off("details"),ratio=w/h;
  // In modalità completa vengono riportati tutti i campi compilati, senza modificarli.
  const cap=d.contentMode==="essential"?Math.min(3,rawSections.length):d.contentMode==="compact"?Math.min(6,rawSections.length):rawSections.length;
  const requested=d.contentMode==="complete"?cap:Math.min(Math.max(1,Number(limit)||cap),cap);
  const items=rawSections.slice(0,requested);
  if(!items.length)return;
  cols=ratio<.82?1:Math.min(Math.max(1,cols),items.length,ratio>1.15?2:1);
  box("details",x+o.x,y+o.y,bw,bh);
  const gap=Math.max(5,unit*.006),rows=Math.ceil(items.length/cols),cw=(bw-gap*(cols-1))/cols;
  const pad=Math.max(7,unit*.008),headBase=Math.max(11,unit*.014);
  let bodySize=Math.max(14,unit*(ratio>1.2?.019:.021));
  const minimum=Math.max(9,unit*.0105);
  function layoutFor(size){
   const headSize=Math.max(minimum,size*.78,headBase*.78),lineHeight=size*1.16;
   const lines=items.map(([,body])=>{ctx.font=`600 ${size}px Arial,sans-serif`;return wrapAllLines(ctx,String(body||""),cw-pad*2)});
   const natural=lines.map(v=>pad+headSize*1.25+Math.max(1,v.length)*lineHeight+pad);
   const heights=[];
   for(let r=0;r<rows;r++){let rh=0;for(let c=0;c<cols;c++){const i=r*cols+c;if(i<natural.length)rh=Math.max(rh,natural[i])}heights.push(rh)}
   return{headSize,lineHeight,lines,heights,total:heights.reduce((a,b)=>a+b,0)+gap*(rows-1)};
  }
  let layout=layoutFor(bodySize);
  while(layout.total>bh&&bodySize>minimum){bodySize-=.5;layout=layoutFor(bodySize)}
  // Se il testo è molto lungo, usa tutto lo spazio disponibile senza troncare.
  // Il contenitore si estende verso l'alto, restando dentro la locandina.
  let drawY=y+o.y,drawH=bh;
  if(layout.total>bh){
   const extra=Math.min(layout.total-bh,Math.max(0,y-m));
   drawY-=extra;drawH+=extra;
   box("details",x+o.x,drawY,bw,drawH);
  }
  let rowHeights=layout.heights;
  if(layout.total<drawH){const extra=(drawH-layout.total)/rows;rowHeights=rowHeights.map(v=>v+extra)}
  items.forEach(([head],i)=>{
   const row=Math.floor(i/cols),col=i%cols,cx=x+o.x+col*(cw+gap),cy=drawY+rowHeights.slice(0,row).reduce((a,b)=>a+b,0)+gap*row,ch=rowHeights[row];ctx.save();
   if(mode==="cards"){ctx.fillStyle=p.surface;rr(cx,cy,cw,ch,unit*.010);ctx.fill();ctx.strokeStyle=p.line;ctx.lineWidth=Math.max(1,unit*.0012);ctx.stroke()}
   else{ctx.fillStyle=p.surface;ctx.globalAlpha=.98;rr(cx,cy,cw,ch,unit*.007);ctx.fill();ctx.globalAlpha=1;ctx.strokeStyle=p.accent;ctx.lineWidth=Math.max(2,unit*.003);ctx.beginPath();ctx.moveTo(cx,cy);ctx.lineTo(cx,cy+ch);ctx.stroke()}
   ctx.fillStyle=tc.details||p.accent;ctx.font=`900 ${layout.headSize}px Arial,sans-serif`;ctx.textAlign="left";ctx.textBaseline="top";ctx.fillText(head,cx+pad,cy+pad);
   const bodyTop=cy+pad+layout.headSize*1.25;ctx.fillStyle=tc.details||p.ink;ctx.font=`600 ${bodySize}px Arial,sans-serif`;
   layout.lines[i].forEach((line,j)=>ctx.fillText(line,cx+pad,bodyTop+j*layout.lineHeight));ctx.restore();
  });
 }
 function drawSafeContentPanel(){
  if(!rawSections.length)return;
  const portrait=h>w;
  const panelX=portrait?m:w*.53;
  const panelY=portrait?h*.565:m;
  const panelW=portrait?w-m*2:w*.47-m;
  const panelH=portrait?h*.365:h-m*2;
  const pad=unit*.022,gap=unit*.012;
  const cols=portrait?2:1;
  const colGap=unit*.018,colW=(panelW-pad*2-colGap*(cols-1))/cols;
  const items=rawSections.map(([head,body])=>({head,lines:String(body||"").split(/\n+/).filter(Boolean)}));
  function calculate(bodySize){
   const headSize=Math.max(unit*.012,bodySize*.72),lineH=bodySize*1.22;
   const blocks=items.map(item=>{
    ctx.font=`600 ${bodySize}px Arial,sans-serif`;
    const wrapped=[];item.lines.forEach(line=>wrapped.push(...wrapAllLines(ctx,line,colW)));
    return {...item,wrapped,height:headSize*1.28+Math.max(1,wrapped.length)*lineH+gap};
   });
   const columns=Array.from({length:cols},()=>({height:0,blocks:[]}));
   blocks.forEach(block=>{const target=columns.reduce((best,c,i,a)=>c.height<a[best].height?i:best,0);columns[target].blocks.push(block);columns[target].height+=block.height});
   return{headSize,lineH,columns,maxHeight:Math.max(...columns.map(c=>c.height))};
  }
  let size=unit*(portrait?.0155:.0145),layout=calculate(size),min=unit*.0095;
  while(layout.maxHeight>panelH-pad*2&&size>min){size-=1;layout=calculate(size)}
  ctx.save();ctx.fillStyle=p.surface;ctx.globalAlpha=.97;rr(panelX,panelY,panelW,panelH,unit*.016);ctx.fill();ctx.globalAlpha=1;ctx.strokeStyle=p.line;ctx.lineWidth=Math.max(2,unit*.0015);ctx.stroke();
  layout.columns.forEach((column,ci)=>{let cy=panelY+pad;const cx=panelX+pad+ci*(colW+colGap);column.blocks.forEach(block=>{
   ctx.fillStyle=tc.details||p.accent;ctx.font=`900 ${layout.headSize}px Arial,sans-serif`;ctx.textAlign="left";ctx.textBaseline="top";ctx.fillText(block.head,cx,cy);cy+=layout.headSize*1.28;
   ctx.fillStyle=tc.details||p.ink;ctx.font=`600 ${size}px Arial,sans-serif`;block.wrapped.forEach(line=>{ctx.fillText(line,cx,cy);cy+=layout.lineH});cy+=gap;
  })});ctx.restore();
  box("details",panelX,panelY,panelW,panelH);
 }
 function footer(){
  ...
  ctx.fillStyle=tc.footer||p.muted;ctx.font=`600 ${Math.max(10,unit*.013*scale("footer"))}px Arial`;ctx.textAlign="left";ctx.textBaseline="middle";ctx.fillText(fitEllipsis(ctx,ft,w-m*2),m+o.x,h-m*.52+o.y);
 }
 ...
 async function drawExternalPoster(){
  ctx.fillStyle=p.bg;ctx.fillRect(0,0,w,h);
  if(img){const sc=Math.max(w/img.width,h/img.height),dw=img.width*sc,dh=img.height*sc;ctx.drawImage(img,(w-dw)/2,(h-dh)/2,dw,dh)}
  const cls=formatClass(d.type),requested=d.externalTextPosition||"auto";
  let pos=requested;
  if(pos==="auto")pos=(cls==="landscape"||cls==="ultraWide"||cls==="printLandscape")?"right":(cls==="portrait"||cls==="printPortrait")?"bottom":"bottom";
  const pad=unit*.052,gap=unit*.018;
  let x=pad,y=pad,bw=w-pad*2,bh=h*.42;
  if(pos==="left"||pos==="right"){bw=w*.43;bh=h-pad*2;x=pos==="right"?w-bw-pad:pad;y=pad}
  else if(pos==="top"){x=pad;y=pad;bw=w-pad*2;bh=h*.38}
  else{x=pad;y=h-h*.43-pad;bw=w-pad*2;bh=h*.43}
  const panel=d.externalTextPanel||"soft",intensity=clamp(Number(d.externalPanelIntensity??75),0,100)/100,opacity=clamp(Number(d.externalPanelOpacity??85),0,100)/100;
  if(panel!=="none"&&opacity>0){
   ctx.save();
   if(panel==="solid"){ctx.fillStyle=p.surface;ctx.globalAlpha=opacity;rr(x,y,bw,bh,unit*.022);ctx.fill()}
   else{const g=(pos==="left"||pos==="right")?ctx.createLinearGradient(x,y,x+bw,y):ctx.createLinearGradient(x,y,x,y+bh);const a1=Math.min(1,opacity*(.35+.65*intensity)),a2=Math.min(1,opacity*(.08+.54*intensity));g.addColorStop(0,`rgba(5,12,18,${a1})`);g.addColorStop(1,`rgba(5,12,18,${a2})`);ctx.fillStyle=g;rr(x,y,bw,bh,unit*.022);ctx.fill()}
   ctx.restore();
  }
  const darkPanel=panel!=="solid",ink=darkPanel?"#ffffff":p.ink,muted=darkPanel?"rgba(255,255,255,.88)":p.muted;
  const tx=x+pad*.72,tw=bw-pad*1.44;
  badgeDraw(tx,y+pad*.62,Math.min(tw,unit*.44),panel==="none");
  const titleY=y+pad*.62+unit*.065;
  text("title",title,tx,titleY,tw,bh*.34,unit*(cls==="ultraWide"?.050:.060),unit*.024,900,ink);
  const sub=[d.slogan,d.subtitle].filter(hasText).join(" — ");
  text("subtitle",sub,tx,titleY+bh*.34+gap,tw,bh*.18,unit*.020,unit*.011,600,muted);
  const detail=[d.details,d.benefits].filter(hasText).join(" · ");
  text("details",detail,tx,titleY+bh*.54+gap,tw,bh*.17,unit*.016,unit*.009,500,muted);
  button(tx,y+bh-pad*.70-unit*.058,Math.min(tw,unit*.46),panel==="solid");
  footer();
  await drawLogos();
 }
 if(d.externalPosterMode&&img){await drawExternalPoster();return canvas}
 ctx.fillStyle=p.bg;ctx.fillRect(0,0,w,h);
 ...
 if(model===0){ // Hero fotografico
  ...
  badgeDraw(m,m,w*split-m*2);text("title",title,m,h*.21,w*split-m*2,h*.26,unit*.058,unit*.026,900,p.ink);text("subtitle",subtitle,m,h*.50,w*split-m*2,h*.15,unit*.021,unit*.012,500,p.muted);button(m,h*.77,w*split-m*2);if(rawSections.length)cardGrid(w*split+m,h*.62,w*(1-split)-m*2,h*.25,1,2,"lines");
  ...
  if(rawSections.length)cardGrid(m*1.5,h*.67,w-m*3,h*.09,4,4,"lines");button((w-Math.min(w*.38,unit*.44))/2,h*.78,Math.min(w*.38,unit*.44));
  ...
  text("title",title,w*.60,h*.20,w*.34,h*.29,unit*.052,unit*.024,900,p.ink);text("subtitle",subtitle,w*.60,h*.52,w*.32,h*.15,unit*.019,unit*.011,500,p.muted);if(rawSections.length)cardGrid(w*.60,h*.66,w*.32,h*.15,1,2,"lines");button(w*.60,h*.84,w*.30);
  ...
  drawImage(0,0,w,h,.48);decorations("cinematic");badgeDraw((w-w*.34)/2,m,w*.34,true);
  const hasBlocks=rawSections.length>0;
  text("title",title,m,hasBlocks?h*.29:h*.35,w-m*2,hasBlocks?h*.20:h*.24,unit*.071,unit*.030,900,"#fff","center");
  text("subtitle",subtitle,w*.14,hasBlocks?h*.51:h*.62,w*.72,hasBlocks?h*.09:h*.11,unit*.021,unit*.012,500,"#fff","center");
  if(hasBlocks)cardGrid(w*.08,h*.63,w*.84,h*.17,portrait?1:2,portrait?2:3,"lines");
  button((w-w*.36)/2,hasBlocks?h*.84:h*.80,w*.36);
  ...
  if(rawSections.length)cardGrid(m,h*.53,w-m*2,h*.35,2,4,"cards");button(m,h*.90,w*.34);
 }
 drawSafeContentPanel();
...
function loadProject(id){const d=getProjects().find(x=>x.id===id);if(!d)return;["project","type","externalTextPosition","externalTextPanel","externalPanelIntensity","externalPanelOpacity","discipline","objective","audience","tone","tone2","toneMix","style","styleMode","logoSize","logoScale","logo2Size","logo2Scale","logo2Position","logoBgMode","logoTolerance","logo2BgMode","logo2Tolerance","contentMode","sloganInput","subtitleInput","details","benefits","program","targetText","methodText","includedText","requirementsText","notesText","eventTime","eventSeats","eventPrice","eventPhone","contact","ctaType","ctaValue","ctaMessage","date","location"].forEach(k=>{if(d[k]!=null&&$(k))$(k).value=d[k]});$("titleInput").value=d.title||"";if($("externalPosterMode"))$("externalPosterMode").checked=d.externalPosterMode===true;$("sloganInput").value=d.slogan||"";$("subtitleInput").value=d.subtitle||"";$("accentColor").value=d.accent||"#ff7a00";$("goldColor").value=d.gold||"#d8ab4d";applyPaletteValues(d.palette||{accent:d.accent,gold:d.gold},false);textColors={...DEFAULT_TEXT_COLORS,...(d.textColors||{})};syncTextColorPicker();syncProtectionLabels();currentImage=d.image||"";currentLogo=d.logo||"";currentLogoOriginal=d.logoOriginal||d.logo||"";currentLogo2=d.logo2||"";currentLogo2Original=d.logo2Original||d.logo2||"";extraLogos=Array.isArray(d.extraLogos)?d.extraLogos:[];if($("ctaPdfEnabled"))$("ctaPdfEnabled").checked=d.ctaPdfEnabled!==false;if($("ctaSocialEnabled"))$("ctaSocialEnabled").checked=d.ctaSocialEnabled!==false;updateCtaControls();if($("logoBgMode")&&d.logoBgMode)$("logoBgMode").value=d.logoBgMode;if($("logoScale")&&d.logoScale!=null){$("logoScale").value=d.logoScale;$("logoScaleValue").textContent=d.logoScale+"%"}if($("logo2Scale")&&d.logo2Scale!=null){$("logo2Scale").value=d.logo2Scale;$("logo2ScaleValue").textContent=d.logo2Scale+"%"}if($("logoTolerance")&&d.logoTolerance!=null){$("logoTolerance").value=d.logoTolerance;$("logoToleranceValue").textContent=d.logoTolerance}if($("logo2Tolerance")&&d.logo2Tolerance!=null){$("logo2Tolerance").value=d.logo2Tolerance;$("logo2ToleranceValue").textContent=d.logo2Tolerance}graphicVariant=normalizeGraphicVariant(d.type,d.graphicVariant??0);manualOffsets=normalizeOffsets(d.manualOffsets);elementScales=normalizeScales(d.elementScales);updatePreviewGraphicStatus();updateSelectedScaleControl();applyMedia();updateOffsetReadout();build();closeDrawer();notify("Progetto aperto.")}
...
function newProject(){if(!confirm("Creare un nuovo progetto?"))return;$("project").value="Nuovo progetto K9";$("type").value="Locandina A4 verticale";$("discipline").value="Mantrailing";$("objective").value="Promuovi servizio";$("audience").value="Proprietari di cani";$("tone").value="Professionale";$("tone2").value="Nessuno";$("toneMix").value=70;$("toneMixValue").textContent="70";$("style").value="Premium nero oro";if($("externalPosterMode"))$("externalPosterMode").checked=false;if($("externalTextPosition"))$("externalTextPosition").value="auto";if($("externalTextPanel"))$("externalTextPanel").value="soft";if($("externalPanelIntensity"))$("externalPanelIntensity").value=75;if($("externalPanelOpacity"))$("externalPanelOpacity").value=85;syncProtectionLabels();$("styleMode").value="auto";$("logoSize").value="auto";$("logoScale").value=100;$("logoScaleValue").textContent="100%";$("logo2Size").value="auto";$("logo2Scale").value=100;$("logo2ScaleValue").textContent="100%";$("logo2Position").value="tl";$("logoBgMode").value="auto";$("logoTolerance").value=42;$("logoToleranceValue").textContent="42";$("logo2BgMode").value="auto";$("logo2Tolerance").value=42;$("logo2ToleranceValue").textContent="42";$("contentMode").value="complete";$("titleInput").value="";$("sloganInput").value="";$("subtitleInput").value="";$("details").value="";$("benefits").value="";$("program").value="";$("targetText").value="";$("methodText").value="";$("includedText").value="";$("requirementsText").value="";$("notesText").value="";$("eventTime").value="";$("eventSeats").value="";$("eventPrice").value="";$("eventPhone").value="";$("contact").value="";$("ctaType").value="whatsapp";$("ctaValue").value="";$("ctaMessage").value="Ciao, vorrei ricevere informazioni su questo progetto.";$("ctaPdfEnabled").checked=true;$("ctaSocialEnabled").checked=true;updateCtaControls();$("location").value="";textColors={...DEFAULT_TEXT_COLORS};applyPaletteValues(BUILTIN_PALETTES.professional,false);syncTextColorPicker();currentImage=currentLogo=currentLogoOriginal=currentLogo2=currentLogo2Original="";extraLogos=[];activeLogoKey="logo";graphicVariant=0;updatePreviewGraphicStatus();manualOffsets=normalizeOffsets();elementScales=normalizeScales();updateSelectedScaleControl();applyMedia();updateOffsetReadout();build(false);notify("Nuovo progetto pronto.")}
...
function generateContent(part="all",force=false){
 const service=$("discipline").value,tone=$("tone").value,tone2=$("tone2").value,mix=Number($("toneMix").value||70);
 const set=(id,value)=>{const el=$(id);if(!el||value==null)return;const next=String(value).trim();if(next)el.value=next};
 ...
 if(part==="all"||part==="subtitle")set("subtitleInput",compactGeneratedLine(randomItem(poolFor(service,"subtitles"),force?"":$("subtitleInput").value),120));
 if(part==="all"||part==="description")set("details",compactGeneratedLine(toneBlend(randomItem(poolFor(service,"descriptions"),force?"":$("details").value),tone,tone2,mix),135));
 ...
 if(part==="all"||part==="cta")set("contact",compactGeneratedLine(randomItem(poolFor(service,"ctas"),force?"":$("contact").value),90));
 scheduleLiveBuild(true);
}
...
let liveBuildTimer=0;
function scheduleLiveBuild(immediate=false){
 clearTimeout(liveBuildTimer);
 const run=()=>{liveBuildTimer=0;build(false)};
 if(immediate)run();else liveBuildTimer=setTimeout(run,90);
}
function bindReliableCommands(){
 // Evita submit involontari e rende tutti i pulsanti realmente azionabili su mobile.
 document.querySelectorAll("button:not([type])").forEach(button=>button.type="button");
 const textPanel=document.querySelector('.tab-panel[data-panel="texts"]');
 if(textPanel){
  const refresh=event=>{if(event.target.matches("input, textarea, select"))scheduleLiveBuild(false)};
  textPanel.addEventListener("input",refresh,true);
  textPanel.addEventListener("change",refresh,true);
  textPanel.addEventListener("paste",()=>setTimeout(()=>scheduleLiveBuild(true),0),true);
  textPanel.addEventListener("compositionend",()=>scheduleLiveBuild(true),true);
 }
}
bindReliableCommands();
