window.__K9_ERRORS=[];
window.addEventListener("error",e=>{window.__K9_ERRORS.push(String(e.message||e.error||"Errore sconosciuto"));document.documentElement.dataset.k9Error=window.__K9_ERRORS.join(" | ")});
window.addEventListener("unhandledrejection",e=>{window.__K9_ERRORS.push(String(e.reason||"Promise rifiutata"));document.documentElement.dataset.k9Error=window.__K9_ERRORS.join(" | ")});
...
async function renderDesign(canvas,d){
 const token=++renderToken,f=formats[d.type],baseStyle=styles[d.style]||styles["Premium tattico"],visualStyle=styleWithMode(baseStyle,d.styleMode||"auto"),graphic=selectedGraphic(d.type,d.graphicVariant??graphicVariant),s={...visualStyle,layout:graphic[1]||visualStyle.layout},w=f.w,h=f.h;
 canvas.width=w;canvas.height=h;
 const ctx=canvas.getContext("2d",{alpha:false});ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality="high";
 const [img,logo,logo2,...extraLogoImages]=await Promise.all([loadImg(currentImage),loadImg(currentLogo),loadImg(currentLogo2),...extraLogos.map(l=>loadImg(l.data))]);if(token!==renderToken&&canvas.id==="previewCanvas")return;
 ...
 function drawLogoForegroundNow(pos="br",max=.12,back=true){
  ...
  ctx.drawImage(logo,x,y,lw,lh);ctx.restore();canvas._logoBoxes=canvas._logoBoxes||{};canvas._logoBoxes.logo={x,y,w:lw,h:lh};
 }
 ...
 function drawSecondLogoForeground(){
  ...
  ctx.drawImage(logo2,x,y,lw,lh);ctx.restore();canvas._logoBoxes=canvas._logoBoxes||{};canvas._logoBoxes.logo2={x,y,w:lw,h:lh};
 }
...
let pendingForegroundLogo=null;
 ...
 function drawAdditionalLogos(){
  extraLogos.forEach((slot,i)=>{
   ...
   canvas._logoBoxes=canvas._logoBoxes||{};canvas._logoBoxes[key]={x,y,w:lw,h:lh};
  });
 }
...
function flushForegroundLogo(){
 ...
 function richInfoPanel(){
  ...
  roundedRect(ctx,px,py,pw,ph,radius);ctx.fill();
  ...
  const bodyFs=clamp(unit*(portrait?.021:.0195)*typeBoost*panelTypographyScale,16*scale,38*scale);
  ...
  function section(x,y,title,items,bottomLimit,limit=4){
   ...
   for(let i=0;i<Math.min(items.length,limit);i++){
    const ls=wrapLines(ctx,items[i],colW-bodyFs*1.10,2);
    const need=ls.length*lineH+bodyFs*.30;
    if(yy+need>bottomLimit)break;
  ...
  availableSections.forEach((entry,index)=>{
    ...
    section(sx,sy,entry.title,entry.items,bottom,complete?(portrait?2:3):2);
  });
  ctx.restore();
...
function k9v4Palette(d){
 const base=styles[d.style]||styles['Premium tattico'];return {accent:d.accent||base.accent||'#f47421',gold:d.gold||base.gold||'#d7ae62',dark:'#0b1016',ink:'#17202a',paper:'#f7f5f1',muted:'#66717c',light:!!base.light,bg1:base.bg1||'#101820',bg2:base.bg2||'#374451'};
}
...
async function renderDesignV4(canvas,d){
 const token=++renderToken,f=formats[d.type]||formats['Post Instagram quadrato'],w=f.w,h=f.h;canvas.width=w;canvas.height=h;
 const ctx=canvas.getContext('2d',{alpha:false});ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';
 const [img,logo,logo2,...extras]=await Promise.all([loadImg(currentImage),loadImg(currentLogo),loadImg(currentLogo2),...extraLogos.map(l=>loadImg(l.data))]);if(token!==renderToken&&canvas.id==='previewCanvas')return canvas;
 ...
 const styleHash=k9v4Hash(d.style+"|"+d.type);let family=styleHash%8;const decorVariant=Math.floor(styleHash/8)%12;if(portrait&&family===3)family=1;if(wide&&family===6)family=2;
 ...
 if(sub&&ty<((family===2||family===4||family===5)?content.y+th:hero.y+hero.h-m)){const fit=k9v4Fit(ctx,sub,tw,Math.max(unit*.08,th-(ty-(family===2||family===4||family===5?content.y:hero.y))),Math.round(unit*(portrait?.034:.031)),Math.round(unit*.022),600,4);k9v4DrawText(ctx,fit,tx,ty,textColor==='white'?'#fff':textColor,'left')}
 // Content region recalculated independently for every format.
...
async function build(autoTitle=false){
 const fallback=$("renderFallback"),fallbackText=$("renderFallbackText");
 try{
  if(fallback)fallback.classList.add("hidden");
  const d=projectData(autoTitle);
  updateTexts(d);
  await renderDesignV4($("previewCanvas"),d);
...
function getProjects(){try{return JSON.parse(localStorage.getItem(STORAGE_KEY)||"[]")}catch{return[]}}function setProjects(v){localStorage.setItem(STORAGE_KEY,JSON.stringify(v));renderSaved()}
...
async function exportImage(mime,ext,quality=1){const d=projectData(false),c=document.createElement("canvas");await renderDesignV4(c,d);c.toBlob(blob=>{if(!blob)return notify("Esportazione non riuscita.",true);const u=URL.createObjectURL(blob),a=document.createElement("a");a.href=u;a.download=`${safeName(d.project)}-${c.width}x${c.height}.${ext}`;a.click();setTimeout(()=>URL.revokeObjectURL(u),1000);notify(`${ext.toUpperCase()} esportato: identico all’anteprima.`)},mime,quality)}
...
async function exportPdfDirect(){
 const d=projectData(false),c=document.createElement("canvas");await renderDesignV4(c,d);
 ...
 parts.push(asciiBytes(xref));const pdf=concatBytes(parts),blob=new Blob([pdf],{type:"application/pdf"}),u=URL.createObjectURL(blob),a=document.createElement("a");a.href=u;a.download=`${safeName(d.project)}-${c.width}x${c.height}.pdf`;a.click();setTimeout(()=>URL.revokeObjectURL(u),1200);notify(hasLink?"PDF generato con pulsante cliccabile.":"PDF generato direttamente dall’app.")
}
...
async function makeSocialCanvas(channel){
 ...
 const canvas=document.createElement("canvas");await renderDesignV4(canvas,socialData);return{canvas,cfg,d,socialData};
}
...
let activeLogoDrag=null;
function beginLogoDrag(event){
 if(!$("freeLogoDragEnabled")?.checked)return;
 const canvas=$("previewCanvas"),key=selectedDragLogo(),box=canvas?._logoBoxes?.[key];
 if(!canvas||!box)return;
 const p=canvasPointFromEvent(canvas,event);
 if(p.x<box.x||p.x>box.x+box.w||p.y<box.y||p.y>box.y+box.h)return;
 activeLogoDrag={key,startX:p.x,startY:p.y,offsetX:manualOffsets[key]?.x||0,offsetY:manualOffsets[key]?.y||0};
 canvas.setPointerCapture?.(event.pointerId);
 event.preventDefault();
}
function moveLogoDrag(event){
 if(!activeLogoDrag)return;
 const canvas=$("previewCanvas"),p=canvasPointFromEvent(canvas,event),factor=1080/Math.min(canvas.width,canvas.height);
 const o=manualOffsets[activeLogoDrag.key]||(manualOffsets[activeLogoDrag.key]={x:0,y:0});
 o.x=clamp(activeLogoDrag.offsetX+(p.x-activeLogoDrag.startX)*factor,-1080,1080);
 o.y=clamp(activeLogoDrag.offsetY+(p.y-activeLogoDrag.startY)*factor,-1080,1080);
 if($("moveTarget"))$("moveTarget").value=activeLogoDrag.key;
...
preview?.addEventListener("pointerdown",beginLogoDrag);
preview?.addEventListener("pointermove",moveLogoDrag);
preview?.addEventListener("pointerup",endLogoDrag);
preview?.addEventListener("pointercancel",endLogoDrag);
...
$("previewCanvas").addEventListener("click",e=>{const c=e.currentTarget,b=c._ctaBox;if(!b||!b.url)return;const r=c.getBoundingClientRect(),x=(e.clientX-r.left)*c.width/r.width,y=(e.clientY-r.top)*c.height/r.height;if(x>=b.x&&x<=b.x+b.w&&y>=b.y&&y<=b.y+b.h)window.open(b.url,"_blank","noopener,noreferrer")});
