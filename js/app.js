...
function flushForegroundLogo(){
 ...
 function richInfoPanel(){
  ...
  const typography=clamp(elementScale("details"),.82,1.12);
  const benefits=listValues(d.benefits,[]),program=listValues(d.program,[]),targets=listValues(d.targetText,[]);
  ...
  const labels=serviceLabels(d.discipline);
  let sections=[
   benefits.length?{title:labels[0],items:benefits}:null,
   program.length?{title:labels[1],items:program}:null,
   targets.length?{title:labels[2],items:targets}:null,
   method.length?{title:"Metodo di lavoro",items:method}:null,
   included.length?{title:"Cosa è incluso",items:included}:null,
   requirements.length?{title:"Requisiti",items:requirements}:null,
   notes.length?{title:"Note",items:notes}:null,
   info.length?{title:labels[3],items:info}:null
  ].filter(Boolean);
  if(d.contentMode==="essential")sections=sections.slice(0,2);
  else sections=sections.slice(0,4);
  if(!sections.length)return;

  const recipes={
   corporate:{mode:"feature",surface:"light",radius:.018,stroke:"soft"},minimal:{mode:"columns",surface:"paper",radius:.006,stroke:"gold"},
   educational:{mode:"cards",surface:"glass",radius:.022,stroke:"accent"},editorial:{mode:"columns",surface:"dark",radius:.004,stroke:"gold"},
   sport:{mode:"strip",surface:"dark",radius:.010,stroke:"accent"},social:{mode:"cards",surface:"glass",radius:.030,stroke:"accent"},
   luxury:{mode:"columns",surface:"dark",radius:.004,stroke:"gold"},organic:{mode:"feature",surface:"natural",radius:.030,stroke:"gold"},
   industrial:{mode:"strip",surface:"dark",radius:.003,stroke:"accent"},cinematic:{mode:"cards",surface:"dark",radius:.014,stroke:"accent"},
   splitLeft:{mode:"strip",surface:"dark",radius:.006,stroke:"accent"},splitRight:{mode:"feature",surface:"dark",radius:.006,stroke:"accent"},
   topBanner:{mode:"columns",surface:"glass",radius:.014,stroke:"gold"},bottomCard:{mode:"cards",surface:"glass",radius:.020,stroke:"accent"},
   centerPoster:{mode:"feature",surface:"dark",radius:.018,stroke:"gold"},framedPhoto:{mode:"columns",surface:"paper",radius:.008,stroke:"gold"},
   magazine:{mode:"feature",surface:"paper",radius:.003,stroke:"accent"},diagonalCut:{mode:"strip",surface:"dark",radius:.004,stroke:"accent"},
   stackedCards:{mode:"cards",surface:"glass",radius:.026,stroke:"gold"},cornerFocus:{mode:"feature",surface:"dark",radius:.012,stroke:"accent"},
   ribbonEvent:{mode:"strip",surface:"dark",radius:.006,stroke:"gold"},verticalStrip:{mode:"columns",surface:"dark",radius:.004,stroke:"accent"},
   photoWindow:{mode:"feature",surface:"paper",radius:.016,stroke:"gold"},asymGrid:{mode:"feature",surface:"dark",radius:.012,stroke:"accent"},
   eventTicket:{mode:"strip",surface:"paper",radius:.008,stroke:"accent"},spotlight:{mode:"cards",surface:"dark",radius:.020,stroke:"gold"},
   monochrome:{mode:"columns",surface:"dark",radius:.002,stroke:"soft"},neonTech:{mode:"cards",surface:"dark",radius:.006,stroke:"accent"},
   softProfessional:{mode:"cards",surface:"paper",radius:.024,stroke:"soft"},blueprint:{mode:"strip",surface:"dark",radius:.002,stroke:"accent"},
   urbanPoster:{mode:"feature",surface:"dark",radius:.002,stroke:"accent"},retroBadge:{mode:"cards",surface:"natural",radius:.030,stroke:"gold"},
   corporatePremium:{mode:"columns",surface:"paper",radius:.008,stroke:"soft"},photoCollage:{mode:"cards",surface:"glass",radius:.012,stroke:"accent"},
   polaroid:{mode:"feature",surface:"paper",radius:.004,stroke:"gold"},mosaic:{mode:"cards",surface:"dark",radius:.008,stroke:"accent"},
   bigType:{mode:"strip",surface:"dark",radius:.002,stroke:"accent"},tactical:{mode:"cards",surface:"dark",radius:.010,stroke:"accent"},
   operational:{mode:"strip",surface:"dark",radius:.003,stroke:"gold"}
  };
  const recipe=recipes[s.layout]||{mode:"cards",surface:light?"paper":"dark",radius:.014,stroke:"accent"};
  if(portrait&&recipe.mode==="strip")recipe.mode="cards";
  const safe=Math.max(m,unit*.026),px=safe,pw=w-safe*2;
  const preferred=Math.max(richPanelTop,contentBottom+unit*.022);
  const minH=portrait?h*.34:(ultra?h*.33:h*.36);
  const py=clamp(preferred,safe,h-safe-minH),ph=h-py-safe;
  if(ph<unit*.25)return;
  const panelShift=shiftBox("details",px,py,pw,ph),panelX=panelShift.x,panelY=panelShift.y;
  ctx.save();ctx.globalCompositeOperation="source-over";
  let panelFill;
  if(recipe.surface==="paper")panelFill=light?"rgba(255,253,248,.97)":"rgba(244,239,229,.96)";
  else if(recipe.surface==="natural")panelFill=light?"rgba(241,238,225,.96)":"rgba(17,28,21,.96)";
  else if(recipe.surface==="glass")panelFill=light?"rgba(255,255,255,.88)":"rgba(8,13,20,.86)";
  else panelFill=light?"rgba(248,249,250,.96)":"rgba(3,7,12,.96)";
  ctx.fillStyle=panelFill;roundedRect(ctx,panelX,panelY,pw,ph,unit*recipe.radius);ctx.fill();
  ctx.strokeStyle=recipe.stroke==="gold"?gold:(recipe.stroke==="soft"?(light?"rgba(20,35,45,.20)":"rgba(255,255,255,.18)"):accent);
  ctx.lineWidth=Math.max(2,unit*.0022);ctx.stroke();registerElementBox("details",panelX,panelY,pw,ph,"left");

  const pad=unit*(portrait?.026:.022),gap=unit*(portrait?.014:.012);
  const actionH=hasCtaInfo(d)?clamp(unit*.060,unit*.050,unit*.074):0;
  const actionTop=panelY+ph-actionH-(actionH?unit*.014:0);
  const gridX=panelX+pad,gridY=panelY+pad,gridW=pw-pad*2,gridH=actionTop-gridY-(actionH?unit*.016:pad*.25);
  let boxes=[];
  if(recipe.mode==="strip"&&!portrait){
   const n=sections.length,cw=(gridW-gap*(n-1))/n; boxes=sections.map((_,i)=>({x:gridX+i*(cw+gap),y:gridY,w:cw,h:gridH}));
  }else if(recipe.mode==="columns"){
   const cols=portrait?1:2,rows=Math.ceil(sections.length/cols),cw=(gridW-gap*(cols-1))/cols,rh=(gridH-gap*(rows-1))/rows;
   boxes=sections.map((_,i)=>({x:gridX+(i%cols)*(cw+gap),y:gridY+Math.floor(i/cols)*(rh+gap),w:cw,h:rh}));
  }else if(recipe.mode==="feature"&&sections.length>2&&!portrait){
   const leftW=gridW*.46,rightW=gridW-leftW-gap; boxes.push({x:gridX,y:gridY,w:leftW,h:gridH});
   const rh=(gridH-gap*(sections.length-2))/(sections.length-1); for(let i=1;i<sections.length;i++)boxes.push({x:gridX+leftW+gap,y:gridY+(i-1)*(rh+gap),w:rightW,h:rh});
  }else{
   const cols=portrait?1:2,rows=Math.ceil(sections.length/cols),cw=(gridW-gap*(cols-1))/cols,rh=(gridH-gap*(rows-1))/rows;
   boxes=sections.map((_,i)=>({x:gridX+(i%cols)*(cw+gap),y:gridY+Math.floor(i/cols)*(rh+gap),w:cw,h:rh}));
  }

  const darkText=recipe.surface==="paper"||light;
  function renderSection(entry,b,i){
   const inset=unit*.012;
   if(recipe.mode==="cards"||recipe.mode==="feature"){
    ctx.fillStyle=darkText?"rgba(255,255,255,.58)":"rgba(255,255,255,.050)";
    roundedRect(ctx,b.x,b.y,b.w,b.h,unit*Math.max(.004,recipe.radius*.62));ctx.fill();
    ctx.strokeStyle=recipe.stroke==="gold"?`${gold}99`:`${accent}88`;ctx.lineWidth=Math.max(1,unit*.0011);ctx.stroke();
   }else if(i>0&&!portrait){ctx.strokeStyle=darkText?"rgba(20,30,40,.14)":"rgba(255,255,255,.13)";ctx.beginPath();ctx.moveTo(b.x-gap*.5,b.y);ctx.lineTo(b.x-gap*.5,b.y+b.h);ctx.stroke()}
   const x=b.x+inset,y=b.y+inset,maxW=b.w-inset*2,maxH=b.h-inset*2;
   const chars=entry.items.join(" ").length;
   let titleFs=clamp(unit*(portrait?.018:.0165)*typography,12*scale,30*scale);
   let bodyFs=clamp(titleFs*.58*(chars>260?.88:1),9*scale,20*scale);
   ctx.textBaseline="top";ctx.textAlign="left";ctx.font=`900 ${titleFs}px Arial`;
   const titleLines=wrapLines(ctx,entry.title.toUpperCase(),maxW,2);
   const titleH=titleLines.length*titleFs*1.08;
   const bodyTop=y+titleH+bodyFs*.52,avail=Math.max(bodyFs*2,maxH-titleH-bodyFs*.55);
   let prepared=[];
   while(bodyFs>=8*scale){
    ctx.font=`600 ${bodyFs}px Arial`;const lh=bodyFs*1.23;
    prepared=entry.items.map(item=>wrapLines(ctx,item,maxW-bodyFs*.72,99));
    const need=prepared.reduce((a,l)=>a+l.length*lh+bodyFs*.20,0);
    if(need<=avail)break;bodyFs-=.5*scale;
   }
   // Heading never overwhelms description: hard ratio cap.
   titleFs=Math.min(titleFs,bodyFs*1.72);
   ctx.font=`900 ${titleFs}px Arial`;ctx.fillStyle=accent;let ty=y;titleLines.forEach(line=>{ctx.fillText(line,x,ty);ty+=titleFs*1.08});
   ctx.font=`600 ${bodyFs}px Arial`;ctx.fillStyle=darkText?"#19232c":"#f4f6f8";let yy=Math.max(bodyTop,ty+bodyFs*.32),lh=bodyFs*1.23;
   prepared.forEach((lines,j)=>{ctx.fillStyle=j%2?gold:accent;ctx.beginPath();ctx.arc(x+bodyFs*.18,yy+bodyFs*.42,Math.max(1.2,bodyFs*.14),0,Math.PI*2);ctx.fill();ctx.fillStyle=darkText?"#19232c":"#f4f6f8";lines.forEach(line=>{ctx.fillText(line,x+bodyFs*.52,yy);yy+=lh});yy+=bodyFs*.20});
  }
  sections.forEach((entry,i)=>renderSection(entry,boxes[i],i));
  ctx.restore();
  drawingRichPanel=true;
  if(actionH)button(panelX+pad,actionTop,portrait?gridW:gridW*(recipe.mode==="strip"?.34:.42),false,false);
  footer("right");drawingRichPanel=false;
 }
