...
async function build(autoTitle=false){
 ...
 try{
  ...
  await renderDesignV4(staging,d);
...
async function exportPdfDirect(){
 const d=projectData(false);if(!allowExport(d))return;const c=document.createElement("canvas");await renderDesignV4(c,d);
