...
async function renderDesign(canvas,d){
 ...
 function cardGrid(x,y,bw,bh,cols=2,limit=4,mode="cards"){
  const o=off("details"),items=rawSections.slice(0,Math.max(limit,rawSections.length));if(!items.length)return;box("details",x+o.x,y+o.y,bw,bh);
  ...
  items.forEach(([head,body],i)=>{const cx=x+o.x+(i%cols)*(cw+gap),cy=y+o.y+Math.floor(i/cols)*(ch+gap);ctx.save();
   ...
   const bodyText=Array.isArray(body)?body.join(" · "):String(body||"");const dense=items.length>4;const r=fitBlock(bodyText,cw-pad*2,ch-pad*2-unit*.026,Math.max(10,unit*(dense?.0115:.0135)),Math.max(7,unit*.0075),500);
