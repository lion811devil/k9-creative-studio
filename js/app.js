...
async function renderDesign(canvas,d){
 ...
 function cardGrid(x,y,bw,bh,cols=2,limit=4,mode="cards"){
  ...
  items.forEach(([head],i)=>{
   ...
   ctx.fillStyle=p.accent;ctx.font=`900 ${layout.headSize}px Arial,sans-serif`;ctx.textAlign="left";ctx.textBaseline="top";ctx.fillText(head,cx+pad,cy+pad);
   const bodyTop=cy+pad+layout.headSize*1.25;ctx.fillStyle=p.ink;ctx.font=`600 ${bodySize}px Arial,sans-serif`;
 ...
 function drawSafeContentPanel(){
  ...
  layout.columns.forEach((column,ci)=>{let cy=panelY+pad;const cx=panelX+pad+ci*(colW+colGap);column.blocks.forEach(block=>{
   ctx.fillStyle=p.accent;ctx.font=`900 ${layout.headSize}px Arial,sans-serif`;ctx.textAlign="left";ctx.textBaseline="top";ctx.fillText(block.head,cx,cy);cy+=layout.headSize*1.28;
   ctx.fillStyle=p.ink;ctx.font=`600 ${size}px Arial,sans-serif`;block.wrapped.forEach(line=>{ctx.fillText(line,cx,cy);cy+=layout.lineH});cy+=gap;
  })});ctx.restore();
 ...
 async function drawExternalPoster(){
  ...
  const panel=d.externalTextPanel||"soft";
  if(panel!=="none"){
   ctx.save();
   if(panel==="solid"){ctx.fillStyle=p.surface;ctx.globalAlpha=.94;rr(x,y,bw,bh,unit*.022);ctx.fill()}
   else{const g=(pos==="left"||pos==="right")?ctx.createLinearGradient(x,y,x+bw,y):ctx.createLinearGradient(x,y,x,y+bh);g.addColorStop(0,"rgba(5,12,18,.92)");g.addColorStop(1,"rgba(5,12,18,.62)");ctx.fillStyle=g;rr(x,y,bw,bh,unit*.022);ctx.fill()}
   ctx.restore();
  }
