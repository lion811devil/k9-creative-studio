...
async function renderDesign(canvas,d){
 ...
 const rawSections=[
  ["DESCRIZIONE",d.details],
  ["VANTAGGI",d.benefits],
  ["PROGRAMMA",d.program],
  ["PER CHI È INDICATO",d.targetText||d.audience],
  ["METODO DI LAVORO",d.methodText],
  ["COSA È INCLUSO",d.includedText],
  ["REQUISITI",d.requirementsText],
  ["NOTE",d.notesText],
  ["INFORMAZIONI EVENTO",[d.eventTime,d.eventSeats,d.eventPrice,d.eventPhone].filter(hasText).join(" · ")],
  ["CONTATTI",d.contact]
 ...
 function cardGrid(x,y,bw,bh,cols=2,limit=4,mode="cards"){
  const o=off("details");
  const ratio=w/h;
  const formatCap=ratio>2?3:ratio>1.2?4:ratio<.82?5:4;
  ...
  if(!items.length)return;box("details",x+o.x,y+o.y,bw,bh);
  ...
  items.forEach(([head,body],i)=>{const cx=x+o.x+(i%cols)*(cw+gap),cy=y+o.y+Math.floor(i/cols)*(ch+gap);ctx.save();
   ...
   const maxBody=Math.max(13,unit*(ratio>1.2?(dense?.014:.016):(dense?.016:.018)));
   const minBody=Math.max(11,unit*(ratio>1.2?.0105:.012));
   const bodyHeight=Math.max(0,ch-pad*2-unit*.032);
   const r=fitBlock(bodyText,cw-pad*2,bodyHeight,maxBody,minBody,500);
   ctx.fillStyle=p.ink;ctx.font=`500 ${r.size}px Arial`;r.lines.forEach((line,j)=>ctx.fillText(line,cx+pad,cy+pad+unit*.031+j*r.size*1.18));ctx.restore();
  });
 }
 function footer(){
...
function generateContent(part="all",force=false){
 ...
 if(part==="all"||part==="description")set("details",compactGeneratedLine(toneBlend(randomItem(poolFor(service,"descriptions"),force?"":$("details").value),tone,tone2,mix),180));
