...
function flushForegroundLogo(){
 ...
 function richInfoPanel(){
  ...
  function section(x,y,title,items,bottomLimit){
   if(bottomLimit-y<headingFs*1.8)return y;
   const titleText=title.toUpperCase();
   let localHeading=headingFs;
   ctx.font=`900 ${localHeading}px Arial`;
   while(localHeading>12*scale&&ctx.measureText(titleText).width>colW){localHeading-=1;ctx.font=`900 ${localHeading}px Arial`;}
   ctx.fillStyle=accent;ctx.textAlign="left";ctx.textBaseline="top";ctx.fillText(titleText,x,y);
   const textWBase=colW-bodyFs*1.10;
   const available=Math.max(20,bottomLimit-(y+localHeading*1.42));
   let localBody=bodyFs,prepared=[];
   while(localBody>=9*scale){
    ctx.font=`600 ${localBody}px Arial`;
    const lh=localBody*1.22;
    prepared=items.map(item=>wrapLines(ctx,item,colW-localBody*1.10,99));
    const needed=prepared.reduce((sum,lines)=>sum+lines.length*lh+localBody*.24,0);
    if(needed<=available)break;
    localBody-=1;
   }
   const lh=localBody*1.22;let yy=y+localHeading*1.42;ctx.font=`600 ${localBody}px Arial`;
   prepared.forEach((lines,i)=>{
    ctx.fillStyle=i%2?gold:accent;ctx.beginPath();ctx.arc(x+localBody*.25,yy+localBody*.43,Math.max(1.5,localBody*.18),0,Math.PI*2);ctx.fill();
    ctx.fillStyle=light?"#18212a":"#f4f6f8";
    lines.forEach(line=>{ctx.fillText(line,x+localBody*.72,yy);yy+=lh;});
    yy+=localBody*.24;
   });
   return yy;
  }
