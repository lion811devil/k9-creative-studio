(() => {
  ...
  function setZoom(value) {
    ...
    canvas.style.setProperty('--preview-zoom', String(state.zoom));
  ...
  function addPreviewControls() {
    ...
    $('#ui2Fit')?.addEventListener('click', () => { setZoom(1); $('#previewCanvas')?.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); });
  ...
  function updateVersion() {
    const footer = $('.footer');
    if (footer) footer.textContent = footer.textContent.replace(/Creative Studio\s+[\d.]+/i, 'Creative Studio 2.0.0');
  }
