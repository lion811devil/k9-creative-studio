(() => {
  ...
  const STORAGE_KEY = 'k9-ui2-settings-v2';
  ...
  function addFocusBar() {
    ...
    bar.innerHTML = `
      <button type="button" class="btn" id="ui2ToggleCompact">Modalità compatta</button>
      <button type="button" class="btn secondary" id="ui2CollapseAll">Comprimi sezioni</button>
      <button type="button" class="btn secondary" id="ui2ExpandAll">Espandi sezioni</button>
      <button type="button" class="btn secondary" id="ui2OpenPreview">Anteprima intera</button>`;
    controls.prepend(bar);

    const compactButton = $('#ui2ToggleCompact');
    const applyCompactLabel = () => {
      if (compactButton) compactButton.textContent = state.compact ? 'Mostra introduzione' : 'Modalità compatta';
    };
    ...
    $('#ui2OpenPreview')?.addEventListener('click', toggleFullscreen);
    applyCompactLabel();
  }
  ...
  function updateVersion() {
    const footer = $('.footer');
    if (footer) footer.textContent = footer.textContent.replace(/Creative Studio\s+[\d.]+/i, 'Creative Studio 7.5');
  }
