...
const safeStorage={get:(k)=>{try{return localStorage.getItem(k)}catch{return null}},set:(k,v)=>{try{localStorage.setItem(k,v);return true}catch{return false}}};
const safeSession={get:(k)=>{try{return sessionStorage.getItem(k)}catch{return null}},set:(k,v)=>{try{sessionStorage.setItem(k,v);return true}catch{return false}}};
...
function readStore(){
 const raw=safeParse(safeStorage.get(STORAGE_KEY),null);
...
function writeStore(store){safeStorage.set(STORAGE_KEY,JSON.stringify(store));}
function activeId(){return safeStorage.get(ACTIVE_KEY)||readStore().kits[0].id;}
function setActiveId(id){safeStorage.set(ACTIVE_KEY,id);}
...
function init(){renderProfiles();fill(activeKit());bind();const k=activeKit();if(k.autoApply!==false&&!safeSession.get("k9.brandKit.applied")){safeSession.set("k9.brandKit.applied","1");setTimeout(()=>applyToProject(false),450)}}
